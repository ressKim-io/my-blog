...
> Alloy → Tempo 직접 전송 + 100% 샘플링 구조에서 5,000 VU 부하테스트를 돌리니 Tempo가 OOMKilled됐습니다. Kafka 버퍼 + tail sampling 10% + 에러/느린 요청 100% 보존으로 해결했습니다.
...
5,000 VU 부하테스트를 실행한 뒤 Grafana를 열었더니 트레이스 관련 패널이 전부 장애였습니다.
...
**85회 재시작**. CrashLoopBackOff 상태에서 exponential backoff 때문에 복구까지 수십 분이 걸렸습니다.
...
130K objects, 95MB 블록을 처리하다 메모리가 터진 것입니다.

---

## 🤔 원인 분석

구조적 문제가 세 가지 겹쳐 있었습니다.

### 1. 100% 샘플링

dev 환경이라 디버깅 편의를 위해 모든 트레이스를 저장하고 있었습니다.
5,000 VU에서 MSA 5서비스가 각각 span을 생성하면 초당 수만 건의 트레이스가 유입됩니다.
Tempo 3Gi 메모리로는 감당이 안 됐습니다.
...
Tempo monolithic 모드에서는 ingestion과 compaction이 같은 Pod에서 실행됩니다.
...
Alloy의 tail sampling 정책을 재설계했습니다.
...
부하테스트 시 트레이스 대부분은 동일한 정상 패턴입니다.
...
`num_traces`를 늘린 이유는, tail sampling은 트레이스가 완료될 때까지 메모리에 들고 있어야 하기 때문입니다.
...
retention을 1시간으로 설정한 이유는, 트레이스 데이터는 Tempo에 저장되면 Kafka에서 삭제해도 되기 때문입니다.
...
Tempo가 감당할 수 있는 속도로 일정하게 데이터를 보내는 것입니다.
부하가 아무리 급증해도 Kafka가 흡수하고, consumer가 일정 속도로 꺼내줍니다.

### 부수 수정: NetworkPolicy

기존에 Alloy → Kafka는 9404(metrics) 포트만 허용되어 있었습니다.
...
HTTP 서버에 rate limiter를 다는 건 당연하게 여기면서, 관측성 파이프라인에는 왜 아무 보호 장치가 없었는지 생각해봐야 합니다.

트레이스 데이터는 애플리케이션 트래픽에 비례해서 증가합니다.
부하테스트처럼 갑자기 트래픽이 10배 뛰면 트레이스도 10배 뜁니다.
**관측 대상이 바빠질 때 관측 시스템도 바빠지는** 구조적 문제가 있습니다.

Kafka 버퍼는 이 문제에 대한 답입니다.
...
100% → 10% 샘플링이라고 하면 "90%를 버린다"고 생각하기 쉽습니다.
하지만 정확히는 **"에러와 느린 요청은 반드시 남기고, 나머지만 통계적으로 줄인다"**입니다.

tail sampling의 장점이 여기 있습니다.
head sampling(확률적)과 달리, 트레이스가 완료된 뒤에 결정하니까 "에러가 있었는지", "SLO를 초과했는지"를 기준으로 판단할 수 있습니다.

### Monolithic Tempo의 한계

Tempo monolithic 모드에서는 ingestion + compaction + query가 한 Pod에서 실행됩니다.
부하가 높을 때 compaction이 메모리를 크게 먹으면서 ingestion과 경쟁합니다.

prod 환경에서는 Tempo distributed mode로 전환해서 ingester, compactor, querier를 분리해야 합니다.

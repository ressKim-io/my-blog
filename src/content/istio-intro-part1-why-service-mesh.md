...
모놀리식 구성은 `Client → Single App → Database` 한 줄로 끝납니다. 네트워크 관점의 고민거리도 단순합니다
...
서킷브레이커 정책을 한 줄 바꾸려고 해도 다음 4단계를 모든 서비스에 적용해야 합니다

1. 모든 서비스 코드 수정
2. 각 서비스 테스트
3. 각 서비스 재배포
4. 서비스가 100개면 100번 반복

정책 1개를 바꿀 때마다 전사 배포가 일어나는 구조라 변경 비용이 매우 큽니다.

...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   1. 서비스 간 통신 (Service-to-Service)                        │
│      → Part 1~3에서 배운 mTLS + AuthorizationPolicy             │
│      → 서비스의 '신원'을 확인                                   │
│                                                                 │
│   2. 사용자 → 서비스 (End User Authentication)                  │
│      → JWT 토큰으로 사용자 확인                                 │
│      → 이번 Part 4의 주제!                                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
│   │  (검증)     │               │  (검증)     │                 │
│   │ ┌─────────┐ │               │ ┌─────────┐ │                 │
│   │ │ Secret  │ │  같은 키!     │ │ Public  │ │  검증만 가능   │
│   │ │   Key   │ │               │ │  Key    │ │                 │
│   │ └─────────┘ │               │ └─────────┘ │                 │
│   └─────────────┘               └─────────────┘                 │
│                                                                 │
│   문제: Secret 유출 시          장점: Public Key 노출 OK        │
│         위조 가능!                     위조 불가능              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                                                 │
│      Istio ──GET──→ https://auth.example.com/.well-known/jwks.json
│                                                                 │
│   2. JWKS 응답 예시                                             │
│                                                                 │
│      {                                                          │
...
```
┌─────────────────────────────────────────────────────────────────┐
│         RequestAuthentication만 있을 때                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   요청 1: JWT 없음          → 통과! (JWT 없는 요청도 허용)      │
│   요청 2: 유효한 JWT        → 통과! (JWT 검증 성공)             │
│   요청 3: 잘못된 JWT        → 거부! (JWT 검증 실패 = 401)       │
│                                                                 │
│   ⚠️  JWT가 없으면 그냥 통과시킴!                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   요청 ──→ RequestAuthentication ──→ AuthorizationPolicy ──→ 서비스
│                    │                        │                   │
│                    ▼                        ▼                   │
│                                                                 │
│   Case 1: JWT 없음                                              │
│   ┌─────────────────┐    ┌─────────────────┐                    │
│   │ JWT 없음        │ →  │ requestPrincipals│                   │
│   │ principal = ""  │    │ 매칭 실패        │ → 403 Forbidden   │
│   └─────────────────┘    └─────────────────┘                    │
│                                                                 │
│   Case 2: 잘못된 JWT                                            │
│   ┌─────────────────┐                                           │
...
│   Case 3: 유효한 JWT                                            │
│   ┌─────────────────┐    ┌─────────────────┐                    │
│   │ JWT 검증 성공   │ →  │ requestPrincipals│                   │
│   │ principal 설정  │    │ ["*"] 매칭!     │ → 200 OK           │
│   └─────────────────┘    └─────────────────┘                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   1. 사용자가 Auth Server에서 JWT 발급받음                      │
│                                                                 │
│   2. JWT와 함께 API 요청                                        │
│      Authorization: Bearer eyJhbGciOiJS...                      │
│                                                                 │
│   3. Istio Sidecar (또는 Gateway)가 JWT 검증                    │
│      ┌────────────────────────────────────┐                     │
│      │ RequestAuthentication              │                     │
│      │ - issuer 확인                      │                     │
│      │ - jwksUri에서 Public Key 가져오기  │                     │
│      │ - 서명 검증                        │                     │
│      │ - exp (만료) 확인                  │                     │
│      │ - aud (audience) 확인 (설정 시)    │                     │
│      └────────────────────────────────────┘                     │
│                         │                                       │
│               검증 실패 │ 검증 성공                             │
│                    ↓    │    ↓                                  │
...
│                         │  (issuer/subject)                     │
│                         │                                       │
│   4. AuthorizationPolicy로 인가 결정                            │
│      ┌────────────────────────────────────┐                     │
│      │ AuthorizationPolicy                │                     │
│      │ - requestPrincipals 매칭           │                     │
│      │ - when 조건 (claims) 확인          │                     │
│      └────────────────────────────────────┘                     │
│                         │                                       │
│               매칭 실패 │ 매칭 성공                             │
...
│                         │                                       │
│   5. 애플리케이션에 요청 전달                                   │
│      (forwardOriginalToken: true면 JWT도 함께)                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

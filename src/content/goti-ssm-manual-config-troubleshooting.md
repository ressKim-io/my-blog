...
> 1바이트 부족한 암호화 키, 복붙한 잘못된 비밀번호 — 결국 **IaC 미관리**가 근본 원인이었어요.
...
두 장애 모두 **SSM Parameter Store에 값을 수동으로 등록**하면서 발생했어요.
Terraform으로 관리했다면 코드 리뷰에서 잡혔을 실수들입니다.

더 심각한 점은, observer가 **master DB 유저**로 접속을 시도하고 있었다는 것이다.
비밀번호 오류가 아니었다면, observer가 모든 테이블에 읽기/쓰기 권한을 가진 채 동작했을 거예요.
...
EKS rolling update 후 서비스를 복구하는 과정에서, **goti-payment-sungjeon-prod만** CrashLoopBackOff가 지속되었어요.

다른 14개 Pod는 모두 정상인데, 이 녀석만 계속 죽었습니다.
...
이 문제가 바로 발견되지 않은 이유가 있어요.

`TokenEncryptor`는 **POC 브랜치에만 존재**하는 클래스입니다.
메인 배포 브랜치(`deploy/prod`)에는 이 코드가 없어요.
...
payment-prod, payment-junsang-prod 등 다른 payment 서비스는 `deploy/prod` 브랜치 기반이라 전혀 영향이 없었어요.
성전님의 POC 브랜치에서만 사용하는 대기열 토큰 암호화 기능이었기 때문에, 다른 서비스가 정상이니 문제를 인지하기까지 시간이 걸렸습니다.

### 해결

SSM Parameter Store에서 값을 32바이트로 수정한 뒤, Secret을 재생성하고 Pod를 재시작했어요.
...
prod EKS 환경에서 goti-load-observer가 RDS 접속 시 **매분 3개 poll 쿼리**(seats/orders/matches)가 전부 실패했어요.
...
서비스별로 DB 유저를 분리해뒀는데, **observer만 전용 유저가 없었어요**.
...
코드 버그 하나로 데이터가 변경되거나, 개인정보가 노출될 수 있는 구조였다.
이건 **최소 권한 원칙의 명백한 위반**입니다.
...
여기서 끝이 아닙니다.
observer가 볼 필요 없는 **PII(개인정보) 테이블의 접근을 차단**해야 해요.
...
이렇게 하면 새로운 테이블이 추가될 때마다 수동으로 GRANT를 실행할 필요가 없어요.
물론, 새 PII 테이블이 추가되면 별도로 REVOKE해야 합니다.

### Step 2: Terraform SSM 파라미터 등록 (수동 → IaC)

수동으로 생성했던 SSM 파라미터를 Terraform으로 가져왔어요.
...
복붙 실수가 코드 리뷰에서 걸리게 되는 구조예요.

### Step 3: Terraform drift 수정 (부수 작업)

Terraform import를 하면서 기존 인프라와 코드 간의 drift를 발견했어요.
이 기회에 함께 정리했습니다.
...
`terraform plan`에서 예상치 못한 변경이 나타나고, 결국 아무도 `terraform apply`를 신뢰하지 못하게 돼요.
...
goti_observer 유저로 접속해서 권한을 검증했어요.
...
> SSM Parameter Store에 값을 **수동으로 등록**했다.

수동 등록이 왜 위험한지 정리해봅시다.
...
장애 1에서는 31바이트 문자열을 입력한 걸 **아무도 검증하지 않았어요**.
...
장애 2에서는 payment 비밀번호를 observer에 복붙한 걸 **아무도 리뷰하지 않았어요**.
코드로 관리했다면 `var.observer_db_password`와 `var.payment_db_password`가 다른 변수라는 게 PR에서 명확히 보였을 겁니다.

결국 두 장애 모두 **"사람의 실수를 시스템이 잡지 못한 것"**이 근본 원인이다.
...
암호화 키를 수동으로 관리한다면, **길이 검증은 필수**예요.
...
`terraform plan`을 신뢰할 수 없게 되면, IaC의 의미가 사라져요.

...
**가설 2 — OTLP tracing gRPC blocking**: ArgoCD values에 `otlp.address: otel-collector-front-dev-...:4317`이 설정되어 있었습니다. gRPC 연결이 startup을 blocking하는지 소스 코드를 확인하니 두 호출 경로가 보였습니다

1. `NewServer() → ensureSynced() → WaitForCacheSync()` — 여기서 hang
2. `Init() → Listen() → InitTracer()` — OTLP는 여기서 초기화 (도달 못 함)

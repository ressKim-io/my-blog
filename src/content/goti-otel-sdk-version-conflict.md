...
> Spring Boot의 `io.spring.dependency-management` 플러그인이 OTel SDK 버전을 강제 다운그레이드해서 `ClassNotFoundException`이 발생했다. `platform()` 대신 `dependencyManagement` 블록을 써야 한다.
...
Goti-server에 OTel 관측성을 통합하고 첫 EC2 배포를 진행했어요.
Spring Boot 3.5.10에 `opentelemetry-instrumentation-bom:2.25.0`을 추가한 상태였습니다.

EC2에서 `docker ps`를 확인하니 goti-server만 이상했어요.
...
로컬에서는 정상 빌드되었는데, EC2에서만 크래시가 발생하는 상황이었어요.
...
`opentelemetry-instrumentation-bom:2.25.0`의 POM을 확인했어요.
...
1.59.0을 선언했는데 **1.49.0으로 다운그레이드**되고 있었어요.
...
`ComponentLoader` 인터페이스는 SDK 1.55.0+에서 도입되었어요.
...
`dependencyManagement` 블록의 `mavenBom`으로 선언해야 Spring Boot BOM보다 우선순위를 가져요.
...
`platform()`은 Gradle 네이티브 BOM 지원 기능이에요.
...
이것이 Spring Boot + OTel 조합에서 가장 흔한 함정이다.

### 재발 방지

OTel 버전을 업그레이드할 때마다 아래 명령어로 다운그레이드 여부를 반드시 확인해야 해요.

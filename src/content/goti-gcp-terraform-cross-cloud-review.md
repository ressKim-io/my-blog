...
> GCP Terraform PR에서 **GKE Master API 전체 개방**, **PGPASSWORD 인증 버그**, **비밀번호 평문 노출** 등 CRITICAL 3건을 포함해 총 9건을 수정했고, K8s manifest가 AWS 전용으로 하드코딩되어 있어 GCP 배포에는 추가 작업이 필요하다.
...
대상 커밋은 `2a7277c`, 브랜치는 `fix/gcp-terraform-hardening`이에요.
...
네트워크, 컴퓨팅, 데이터베이스, 보안, 부트스트랩으로 레이어가 명확하게 나뉘어 있어요.
...
`0.0.0.0/0`이면 **인터넷의 모든 IP**에서 GKE Master API에 접근할 수 있다는 뜻이에요.

이게 왜 위험한지 생각해봅시다.
GKE Master API는 `kubectl` 명령이 도달하는 엔드포인트입니다.
여기에 접근할 수 있으면 클러스터의 모든 리소스를 조회하고, 수정하고, 삭제할 수 있어요.
물론 RBAC 인증이 있지만, **공격 표면 자체를 줄이는 게 첫 번째 방어선**이다.

GKE는 EKS와 달리 Master API가 Public IP로 노출되는 게 기본이에요.
...
환경별 `tfvars`에서 허용할 IP 대역만 명시적으로 지정하면 돼요.
...
데이터베이스 모듈에서 `local-exec`으로 PostgreSQL 초기 설정을 하는 부분이 있었어요.
...
`${random_password.db_user.result}`가 **command 문자열에 직접 삽입**되고 있어요.
이러면 두 가지 경로로 비밀번호가 노출됩니다.

1. **Terraform state**: `local-exec`의 command는 state 파일에 그대로 저장됩니다. state를 열면 비밀번호가 보여요.
2. **프로세스 목록**: `ps aux` 명령으로 실행 중인 프로세스를 보면, command에 비밀번호가 포함되어 있습니다.
3. **CI/CD 로그**: Terraform 실행 로그에 command가 출력될 수 있어요.
...
환경 변수는 **Terraform state에 저장되지 않고**, 프로세스 목록에도 노출되지 않아요.

psql에서 `:'userpass'`는 psql의 변수 참조 문법입니다.
환경 변수 `userpass`의 값을 쿼리에 안전하게 바인딩해줘요.

이 패턴은 `local-exec`에서 비밀번호를 다룰 때 항상 사용해야 하는 기본 패턴이다.

---

## 🟡 HIGH: 보호 장치가 빠져있다

CRITICAL만큼 즉각적인 위험은 아니지만, **운영 사고를 예방하는 장치**가 빠져 있었어요.
...
prod 환경에서 클러스터가 삭제되면 복구가 불가능해요.
모든 워크로드, PV, ConfigMap이 사라집니다.

"개발 중이니까 false로 해놨겠지" 하고 넘어갈 수 있지만, 이런 설정이 **그대로 prod에 올라가는 사고가 실제로 발생한다**.
...
개발 환경에서만 명시적으로 `false`를 전달하면 돼요.
...
`immutable_tags = false`면 **같은 태그로 다른 이미지를 덮어쓸 수 있어요**.
...
GKE는 기본적으로 시스템 로그만 수집해요.
**누가 어떤 리소스를 생성/수정/삭제했는지** 기록하는 Audit 로그는 별도 설정이 필요합니다.

보안 사고가 발생했을 때 Audit 로그가 없으면 **원인 추적이 불가능**하다.
...
GCP Terraform 작업이 시작된 시점과 실제 PR이 올라온 시점 사이에 AWS 쪽에서 버전 업그레이드가 진행되었기 때문이에요.
브랜치를 오래 들고 있으면 이런 일이 발생합니다.

특히 **Istio 1.24.3과 1.29.0은 메이저 버전 차이**예요.
같은 팀에서 운영하는 AWS/GCP 클러스터의 Istio 버전이 다르면, Service Mesh 설정의 호환성 문제가 발생할 수 있습니다.

이런 드리프트를 방지하려면 **version-matrix.md** 같은 단일 소스를 관리하고, 양쪽 Terraform에서 참조하는 방식이 필요해요.
...
Terraform(IaC) 레이어는 AWS/GCP 각각 독립적으로 준비되어 있어요.
하지만 K8s manifest(Goti-k8s, Goti-monitoring)는 **AWS 전용으로 하드코딩**되어 있었습니다.

GCP 클러스터에서 ArgoCD App-of-Apps를 배포하면, 여기저기서 에러가 발생할 거예요.

전체 구조를 ASCII 다이어그램으로 봅시다.
...
Terraform은 클라우드별로 분리되어 있지만, **그 위에서 동작하는 K8s manifest는 한 벌**이에요.
그 한 벌이 AWS 전용으로 작성되어 있다는 게 문제입니다.

6가지 호환성 이슈를 하나씩 살펴봅시다.
...
StorageClass는 **클러스터에서 PVC가 생성될 때 어떤 스토리지를 프로비저닝할지** 결정하는 리소스예요.
...
StatefulSet을 사용하는 모든 워크로드(Mimir, Loki, Redis 등)가 시작하지 못해요.
...
**모든 서비스의 Helm values에 ECR 주소가 하드코딩**되어 있어요.
서비스가 10개라면 10개 파일을 모두 수정해야 합니다.

이걸 매번 수동으로 바꾸는 건 실수가 생기기 쉬워요.
...
다행히 GCP bootstrap Terraform에서 `gcp-sm` ClusterSecretStore는 **이미 생성되어 있어요**.
...
서비스별 ExternalSecret을 하나하나 수정하는 것보다, **overlay로 분기**하는 게 효율적이에요.
...
모니터링 스택의 장기 스토리지가 전부 S3로 설정되어 있어요.
Mimir(메트릭), Loki(로그) 모두 S3 버킷에 데이터를 저장하도록 되어 있습니다.

GCP에서는 GCS 버킷을 사용해야 하고, 설정 형식도 완전히 달라요.
...
GCP에서는 Workload Identity로 Pod에 GCP Service Account를 매핑해요.

**기능은 같지만 메커니즘이 완전히 다르다.**
...
Helm values에서 SA annotation을 변수로 분리하면, 클라우드별로 다른 값만 주입할 수 있어요.
...
OTel Collector의 exporter는 수집한 텔레메트리 데이터를 **어디에 저장할지** 결정하는 컴포넌트예요.
`awss3` exporter는 AWS SDK를 사용하기 때문에 GCP에서는 동작하지 않습니다.

GCP 환경에서는 `googlecloudstorage` exporter로 교체해야 해요.
...
**모듈 구조 (9/10)**: AWS Terraform과 동일한 패턴으로 모듈이 분리되어 있어요. network → compute → database → bootstrap 순서로 의존성이 명확하고, 변수 네이밍도 일관성이 있습니다. 1점 감점은 output 정리가 일부 빠져 있어서예요.

**보안 (5/10 → 8/10)**: 초기 상태에서는 Master API 전체 개방, PGPASSWORD 버그 등 CRITICAL 이슈가 있었습니다. 수정 후에는 대부분 해소되었지만, **Network Policy 기본 정책**이나 **Pod Security Standards** 설정이 아직 없어서 8점이에요.

**버전 호환성 (4/10 → 9/10)**: Istio 5버전 차이, ESO/ArgoCD 메이저 버전 차이가 있었습니다. 수정 후 AWS와 동일한 버전으로 정렬되었어요. 향후 version-matrix.md를 단일 소스로 관리하면 10점이 될 수 있습니다.

**K8s manifest 호환성 (3/10)**: 이건 Terraform PR 범위 밖의 문제이긴 합니다. 하지만 **GCP에서 실제로 서비스를 배포하려면 반드시 해결해야 하는 블로커**예요. 6개 카테고리에서 호환성 이슈가 발견되었고, overlay 구조를 잡는 작업이 필요합니다.

**운영 준비도 (7/10)**: Cloud SQL의 HA 구성, 자동 백업, KMS 암호화가 잘 설정되어 있어요. 감점 요인은 모니터링 스택(Mimir, Loki)의 GCP 연동이 아직 안 되어 있다는 점입니다.
...
**Terraform 보안 체크리스트**:
- `0.0.0.0/0`은 어떤 리소스에서든 **즉시 레드 플래그**다
- `local-exec`에서 비밀번호는 반드시 `environment` 블록으로 분리
- `deletion_protection`은 **기본 true**, 예외적으로만 false

**Cross-Cloud IaC의 현실**:
- Terraform 레이어가 클라우드별로 분리되어 있어도 **K8s manifest가 하드코딩되면 의미 없다**
- 멀티 클라우드를 진지하게 고려한다면, **manifest 레이어의 추상화**가 더 중요하다
- overlay/values 분리 + ApplicationSet 분기가 현실적인 해결책

**코드 리뷰의 가치**:
- PGPASSWORD 버그는 **코드를 한 줄씩 따라가지 않으면 발견할 수 없는 문제**였다
- 변수 이름이 비슷할 때(`db` vs `db_user`) 실수가 발생하기 쉽다
- 보안 이슈는 "나중에 고치자"가 아니라 **merge 전에 반드시 수정**해야 한다

...
Prometheus 데이터소스 자체에 접근하지 못하는 상황이었어요.
...
하지만 Grafana의 데이터소스는 `access: proxy` 모드로 설정되어 있어요.

이 모드에서는 Grafana 서버가 **Docker 내부 네트워크**에서 직접 Prometheus와 통신합니다.
브라우저 → ALB → Prometheus 경로가 아니라, Grafana 컨테이너 → Prometheus 컨테이너 경로예요.
ALB 포트 노출과는 무관합니다.
...
ALB 같은 리버스 프록시가 프로토콜을 변환하면, 이 검증이 실패하는 거예요.
...
이 설정은 "이 도메인에서 오는 요청은 프로토콜이 달라도 CSRF 검증을 통과시켜라"는 의미예요.

### 인프라 반영 과정

단순히 docker-compose만 수정하면 안 됩니다.
EC2 배포 환경에서는 SSM Parameter Store에서 환경변수를 가져오기 때문에, 여러 곳을 수정해야 했어요.
...
config 모듈의 `variables.tf`에는 변수를 선언했지만, **루트 모듈**의 `variables.tf`와 `main.tf`에서 변수를 선언하고 전달하지 않았던 거예요.
...
CLI로 먼저 생성한 SSM 파라미터를 Terraform state에 반영했어요.
...
Django, Rails, Spring Security 등 CSRF 보호가 있는 모든 프레임워크에서 동일하게 발생할 수 있어요.

### 새 도메인 추가 시 잊지 말 것

`GF_SECURITY_CSRF_TRUSTED_ORIGINS`에 새 도메인을 추가해야 합니다.
Go-Ti 프로젝트에서는 이 값을 Terraform `tfvars`에서 관리하기로 했어요.

**프로토콜 변환이 있는 리버스 프록시 뒤에서는 CSRF trusted origins 설정이 필수다.**

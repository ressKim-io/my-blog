...
각 레포지토리마다 CI를 붙여야 하나? 아니면 배포 전용 레포를 따로 만들어야 하나? 고민이 됬습니다.
...
**고민했던 점:**
- 각 레포에 CI를 만들면 둘 다 EC2에 배포해야 하나?
- docker-compose.yml은 어디에 두지?
- 한쪽만 업데이트됐을 때 어떻게 하지?
...
같은 패턴을 K8s 환경으로 확장할 수 있다. 본질은 동일하고 도구만 바뀐다.
...
Docker Compose로 시작해서 K8s로 확장하는 게 이해하기 쉬웠다.
...
실무 패턴을 배우면서 안전성도 확보하는 좋은 방법이었다.

...
문제는 values 파일에서 서비스 주소를 하드코딩할 때 이 suffix를 누락한 것이다.
...
이 표에서 볼 수 있듯이, `-dev` suffix 누락이 7곳에서 동시에 발생했어요.
...
`securityContext.readOnlyRootFilesystem: true`로 설정되어 있는데, Alloy가 `--storage.path=/tmp/alloy`로 쓰기를 시도해요.
...
올바른 키는 `alloy.mounts.extra` + `controller.volumes.extra`예요.
...
Service를 확인해보니 12345/TCP (UI 포트)만 노출되어 있었어요.
...
Alloy에 데이터가 도달하지 않아 Prometheus 메트릭 0건, Loki 로그 미수집 상태였어요.

### 원인

SSM Parameter Store `/dev/server/OTEL_EXPORTER_ENDPOINT`가 EC2 docker-compose 서비스명(`goti-alloy`)으로 설정되어 있었습니다.
Kind 환경에서는 Helm release로 생성된 `alloy-dev.monitoring.svc`를 사용해야 해요.

EC2와 Kind가 **같은 SSM 파라미터를 공유**하고 있는 것이 근본 원인이다.
...
Docker Compose 서비스명과 K8s Service FQDN은 완전히 다른 체계예요.
...
`serviceAccount.automountServiceAccountToken: false`로 설정되어 있었어요.
메인 컨테이너는 SA 토큰이 필요 없지만, sidecar는 K8s API에 접근하여 ConfigMap(alert rule)을 읽어야 합니다.
토큰이 없으니 API 접근이 불가능했던 거예요.
...
메인 컨테이너 기준으로만 판단하면 이런 문제가 발생해요.
...
`{{ $value }}`는 Loki/Prometheus alertmanager 변수이지 Helm 변수가 아니에요.
...
Helm은 모든 `{{ }}`를 자신의 템플릿으로 해석하려고 시도하기 때문이에요.
...
이런 실수를 방지하려면 Helm 변수로 이름을 통일하는 것이 좋아요.
...
ApplicationSet의 `{{component}}-{{env}}` 패턴이 release 이름을 결정하고, 모든 생성 리소스명에 반영돼요.
values에서 서비스 주소를 하드코딩할 때 **반드시 `kubectl get svc -n <ns>`로 실제 이름을 확인**해야 합니다.

### 2. Helm chart values 키는 chart마다 다르다

같은 기능(`extraVolumes`)이라도 chart별로 키 구조가 달라요.

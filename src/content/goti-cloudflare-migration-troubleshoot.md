...
> Cloudflare 전환 과정에서 5가지 라우팅 장애를 만났다. CloudFront Custom Error Response, SSL 이중 종단, Pages/Workers 우선순위, Workers Host 헤더 제한 — 하나를 풀면 다음 문제가 나타나는 연쇄 디버깅이었다.
...
카카오 소셜 로그인 시 프론트엔드에서 이런 에러가 발생했어요.
...
상태 코드는 200이에요. 에러가 아닌 것처럼 보이지만 실제로는 완전히 잘못된 응답이었습니다.

더 혼란스러운 건 **간헐적**으로 발생한다는 거예요.
curl로 같은 엔드포인트를 호출하면 정상 JSON이 돌아왔습니다.
백엔드가 403이나 404를 반환할 때만 트리거됐어요.
...
이건 SPA 라우팅을 위한 설정이에요.
브라우저에서 `/products/123`을 직접 접근하면 S3가 404를 반환하는데, CloudFront가 이걸 `index.html`로 바꿔서 React Router가 처리하게 해주는 거죠.

문제는 이 규칙이 **Distribution 전역**으로 적용된다는 거예요.
...
curl에서는 정상이었던 이유도 설명돼요 — 정상 응답(200, 400, 500)은 Custom Error Response에 해당하지 않으니까요.
...
CloudFront 안에서 해결하려면 Lambda@Edge나 CloudFront Functions로 우회해야 하는데, 이건 근본적인 해결이 아니에요.
...
DNS NS를 Cloudflare로 전환하면서 Proxy(Orange Cloud)가 ON인 상태에서 문제가 발생했어요.
...
Pages는 정적 파일만 서빙하니까 POST 요청에 405를 반환한 거예요.

Cloudflare Pages에 커스텀 도메인을 설정하면, 같은 도메인의 Workers route보다 **Pages가 우선**합니다.
이건 공식 문서에 명확히 나와있지 않아서 직접 겪어야 알 수 있는 부분이에요.
...
Workers가 요청을 가로채는 건 성공했지만, Kind PC로 프록시할 때 또 문제가 생겼어요.
...
// → 404 (751ms) ❌
```
Kind PC까지 도달했어요 (응답 시간 751ms로 확인).
하지만 Istio Gateway가 404를 반환합니다.
Host 헤더가 `resshome.iptime.org`로 전송되니까 Istio가 매칭하지 못한 거예요.
...
curl에서는 Host 헤더 오버라이드가 정상 동작해요.
...
핵심은 `fetch(target, ...)`에서 target URL의 hostname이 `dev-api.go-ti.shop`이라는 거예요.
...
이번 트러블슈팅에서 인상적이었던 건 **문제가 연쇄적으로 나타났다**는 거예요.
...
이건 CDN 전환처럼 여러 레이어가 동시에 바뀌는 작업에서 흔한 패턴이에요.

### 디버깅 원칙: 한 레이어씩 확인

Workers 디버깅에서 가장 도움이 된 건 **한 레이어씩 확인하는 방법**이었어요.

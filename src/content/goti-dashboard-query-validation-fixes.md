...
실제로 Loki에는 goti 로그가 정상 수집되고 있었고, 24시간 기준으로는 5개 서비스 모두 로그가 확인되었습니다.
...
- **변수 치환 누락은 검증 파이프라인 자체의 버그**: extract 단계에서 변수 치환을 빠뜨리면 실제 쿼리가 아닌 literal string을 검증하게 됩니다. 템플릿 변수 목록은 주기적으로 감사해야 합니다.

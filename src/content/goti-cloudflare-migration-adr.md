...
기존에는 CloudFront 한 곳에서 두 갈래로 분기되는 구조였습니다.

- `Client → CloudFront → S3` — React SPA 정적 파일
- `Client → CloudFront → Kind PC` — `/api/*` 경로 API
...
Route53 NS를 Cloudflare NS로 바꾸면서 Cloudflare Proxy(Orange Cloud)가 활성화되자 사이클이 생겼습니다.

1. Client → Cloudflare가 SSL을 종단합니다 (HTTP→HTTPS 리다이렉트)
2. CloudFront가 다시 SSL을 종단하며 HTTP→HTTPS 리다이렉트를 반환합니다
3. Cloudflare가 그 리다이렉트를 따라가며 1번으로 회귀 — 무한 301 루프
...
1. Pages에서 커스텀 도메인을 제거합니다
2. Workers가 모든 라우트를 처리하도록 변경합니다
3. 정적 파일은 Workers에서 Pages `*.pages.dev` URL로 프록시합니다

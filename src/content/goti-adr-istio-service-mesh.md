...
> Istio Sidecar 모드를 선택했다. JWT 검증 + Rate Limiting + mTLS + 분산 트레이싱을 하나의 도구로 해결하기 위해. Ambient Mesh는 L7 관측성 한계로 포기했다.
...
전환 전에 아키텍처를 점검해보니, **부족한 기능이 5가지**나 있었어요.
...
CNCF Graduated 등급의 서비스 메시 3종을 비교했어요.
...
**Istio가 유일하게 JWT + Rate Limiting + mTLS를 모두 제공해요.**
Linkerd는 가볍고 빠르지만, JWT 검증과 Rate Limiting이 없습니다.
결국 별도 도구를 붙여야 하고, 올인원의 이점이 사라져요.

Cilium은 eBPF 기반으로 L3/L4에서는 압도적인 성능을 보여줍니다.
하지만 HTTP 수준의 정책(JWT, Rate Limit)은 Envoy를 추가해야 해요.
그러면 Istio와 복잡도가 비슷해지는데, 생태계는 Istio가 더 성숙합니다.

분산 트레이싱도 차이가 커요.
Istio의 Envoy는 OTel OTLP를 네이티브로 export합니다.
이미 구축한 Alloy → Tempo 파이프라인에 바로 연결되죠.
Linkerd는 Prometheus 메트릭만, Cilium은 L4 위주 메트릭만 제공해요.

### 시나리오별 적합도

모든 상황에서 Istio가 최선은 아닙니다.
시나리오에 따라 최적의 선택이 달라져요.
...
서비스 메시 없이 API Gateway만 도입하는 방안도 검토했어요.
...
Envoy가 각 Pod 옆에 붙어서 L7 트래픽을 직접 보기 때문이에요.
...
ztunnel은 TCP 바이트와 연결 수만 볼 수 있어요.

"그러면 Waypoint를 추가하면 되지 않나?"

맞아요. Waypoint proxy를 추가하면 L7 관측이 가능해집니다.
**하지만 여기서 새로운 문제가 생깁니다.**

### 핵심 문제: 분산 트레이싱 워터폴 뷰의 한계

Goti 프로젝트는 Grafana Tempo의 **워터폴(Waterfall) 트레이스 뷰**가 핵심이에요.
서비스 간 호출 체인을 시간 축으로 펼쳐서 보여주는 뷰입니다.

이 뷰가 의미 있으려면, **각 span에 워크로드 이름이 정확히 표시**되어야 해요.
...
Sidecar 모드에서는 각 span에 서비스 이름이 명확하게 표시돼요.
`goti-gateway → user-service → ticketing-service`로 호출 체인이 한눈에 보입니다.
어디서 지연이 발생했는지 즉시 파악할 수 있어요.

Ambient 모드에서는 `waypoint-proxy-xxxxx`가 서비스 이름 자리를 차지합니다.
실제 워크로드 이름이 불완전하게 표시되거나 누락돼요.

> Istio 1.28+에서 service name 표시가 개선되었지만, trace overlay의 gap은 여전히 존재합니다. 워크로드 이름이 완전히 해결된 것은 아니에요.

티켓 오픈 시 트래픽 폭주가 발생하면, 어떤 서비스에서 병목이 생겼는지 **초 단위로** 파악해야 합니다.
워터폴 뷰에서 서비스 이름이 보이지 않으면, 장애 추적 자체가 불가능해요.

이것이 Ambient를 포기한 가장 큰 이유다.

### 텔레메트리 이중 엣지 문제

Ambient + Waypoint 구성에서는 또 다른 문제가 있어요.

ztunnel(L4)과 waypoint(L7)이 **동시에 텔레메트리를 리포트**합니다.
같은 요청에 대해 두 곳에서 메트릭을 내보내는 거예요.

이로 인해 Kiali 같은 서비스 그래프 도구에서 **트래픽 그래프가 왜곡**됩니다.
실제 요청 1개가 그래프에서 2개로 보이거나,
의도하지 않은 엣지(연결선)가 나타나요.
...
좋은 질문이에요. 실제로 이 부분도 조사했습니다.

Solo.io 문서 기준으로, 커뮤니티 Istio의 ztunnel에서는 L7 HTTP 파싱이 불가능하고 상용 버전에서만 지원되는 것으로 보입니다.

즉, 커뮤니티 Istio를 사용하는 한 ztunnel에서 HTTP 메트릭이나 트레이싱을 얻을 방법이 없어요.
...
모니터링이 안 되면 다음 리소스 계획이나 장애 추적이 불가능해요.
대규모 티켓팅 서비스에서 이는 치명적입니다.

Ambient Mesh의 리소스 절감 이점은 분명합니다.
하지만 2026년 3월 기준, 커뮤니티 Istio의 Ambient 모드는
관측성 측면에서 프로덕션 수준에 도달하지 못했다.

향후 Ambient의 트레이싱 이슈가 해결되면 재평가할 계획이에요.
...
아래 다이어그램은 Istio가 Goti에서 처리하는 기능들의 전체 흐름이에요.
...
외부 트래픽이 들어오면 Istio Ingress Gateway에서 JWT 검증과 Rate Limiting을 먼저 처리해요.
유효하지 않은 토큰은 서비스에 도달하기 전에 차단됩니다.
티켓 오픈 시 트래픽 스파이크도 Gateway 레벨에서 방어해요.

Gateway를 통과한 요청은 mTLS로 암호화되어 서비스로 전달됩니다.
각 서비스의 Envoy sidecar가 L7 메트릭을 자동 수집하고,
OTel trace를 Alloy → Tempo로 export해요.
...
리소스 오버헤드는 Sidecar 모드의 가장 큰 단점이에요.
각 Pod에 Envoy가 붙으면 CPU 10~50m, Memory 128~256Mi가 추가됩니다.

하지만 이 오버헤드는 **예측 가능**합니다.
Pod 수에 비례해서 선형으로 증가하기 때문에, 노드 스펙 계획에 반영할 수 있어요.

반면 관측성이 불완전하면, 장애 시 원인을 찾지 못해 **예측 불가능한 손실**이 발생합니다.
예측 가능한 비용과 예측 불가능한 리스크 중에서, 전자를 선택했다.

---

## 📚 핵심 포인트

1. **서비스 메시 선택은 "무엇이 필요한가"에서 시작한다.** mTLS만 필요하면 Linkerd, L3/L4 정책이면 Cilium, JWT+Rate Limit+mTLS 올인원이면 Istio.

2. **API Gateway만으로는 MSA 보안이 완성되지 않는다.** 게이트웨이 뒤의 서비스 간 통신 보안은 서비스 메시가 필요해요.

3. **Ambient Mesh의 리소스 절감은 매력적이지만, 관측성 비용이 크다.** 특히 분산 트레이싱 워터폴 뷰가 핵심인 프로젝트에서는 Sidecar가 현실적인 선택이에요.

4. **"리소스 효율 vs 관측성"은 트레이드오프다.** Goti에서는 관측성을 선택했어요. 모니터링 없이는 다음 최적화 자체가 불가능하기 때문.

5. **Ambient Mesh는 계속 진화 중이다.** Istio 1.28+에서 개선이 이루어지고 있고, 2026 하반기에 재평가할 계획이에요.

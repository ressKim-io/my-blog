---
title: "Resale FE↔BE API 계약 전수 감사 — 22개 엔드포인트에서 P0 2건·P1 7건 발견"
...
Java→Go 컷오버 이후 Resale 통합 테스트 이전에 API 계약 수준의 불일치를 식별하는 감사가 필요했습니다.
...
Mock 플래그는 BE 구현 전에 FE 개발을 선행할 때 유용하지만, 장기간 유지되면 **계약 불일치를 런타임에서 발견하지 못하는 상태**가 누적됩니다. 이번 감사에서 확인한 P0 2건은 Mock 플래그가 없었다면 훨씬 일찍 표면화됐을 것입니다.
...
이 감사는 코드 수정 없이 **분석 전용**으로 수행했습니다. 실제 수정은 `resale-flow-end-to-end-fix` 편에서 다룹니다.

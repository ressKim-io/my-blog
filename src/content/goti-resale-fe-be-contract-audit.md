...
excerpt: "Goti-front React와 Goti-go resale·payment 서비스 사이의 API 계약을 라우트·요청·응답·enum·타임포맷 단위로 전수 대조했습니다. Mock 플래그에 가려져 있던 P0 2건과 판매자 플로우를 곧장 깨뜨리는 P1 7건을 골라냈습니다"
category: challenge
tags:
  - go-ti
  - Resale
...
> Resale FE↔BE 계약을 22개 엔드포인트 전수 대조한 결과입니다. Mock 플래그 하나가 P0 2건(라우트 미구현·응답 shape 불일치)을 가려두고 있었고, Mock을 거치지 않는 판매자 플로우에서는 P0 1건이 이미 운영 중인 버그로 살아 있었습니다
...
Java→Go 컷오버 이후 Resale 통합 테스트로 들어가기 전에, API 계약 수준의 불일치를 미리 잡아내는 검토가 필요했습니다.
...
그 결과 실서버 계약 불일치가 운영 중에 드러나지 않은 상태였습니다.
...
지금은 Mock 플래그 덕분에 호출 자체가 발생하지 않습니다. 플래그를 끄는 순간 **리셀 마켓 화면 전체가 404로 무너집니다**.
...
Mock 플래그는 BE 구현 전에 FE 개발을 선행할 때 유용하지만, 장기간 유지되면 **계약 불일치를 런타임에 만나지 못하는 상태**가 쌓입니다. 이번 분석에서 확인한 P0 2건은 Mock 플래그가 없었다면 훨씬 일찍 드러났을 것입니다.
...
프론트 `ApiEnvelope<T>` 제네릭은 겉보기엔 안전해 보이지만, T가 배열인지 객체인지는 컴파일 타임에 확인할 수 없습니다. `fetchResaleListingOrderDetails`의 P0 불일치도 TypeScript 타입 레벨에서는 조용히 통과합니다. contract test나 스키마 기반 validation이 없으면 런타임에 `.flat()` 호출 실패로 비로소 드러납니다.

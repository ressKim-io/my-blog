...
> 3개 대기열 구현체를 300/1000/3000 VU로 Saturation 테스트한 결과, 수연은 처리량 최고, 준상은 신뢰성 최고, 성전은 안정성 최고로 각각 다른 축에서 강점을 보였다.
...
Saturation 테스트를 돌리면서 각 구현체의 버그를 발견하고 수정했습니다. 부하를 주기 전까지는 발견할 수 없었던 문제들이에요.

### 준상: Redis SCAN → O(1) 카운터
...
SCAN은 키가 많아질수록 느려집니다. 100개 슬롯이 다 차면 매 요청마다 SCAN을 돌리니까 **슬롯 회전이 멈췄어요**. iterations가 100에서 고정된 이유가 이거였습니다.

O(1) 카운터(RedisCache)로 전환하고, `queueComplete()` API를 추가해서 예매 완료 시 슬롯을 반환하도록 수정했습니다.

결과: iterations가 **100 → 4,476** (300 VU 기준)으로 44배 증가.

### 수연: Lua Script ARGV 직렬화 불일치
...
`RedisTemplate<String, Object>`의 `GenericJackson2JsonRedisSerializer`가 Lua script의 ARGV를 `"\"maxCapacity\""` 형태로 직렬화했어요. Redis Hash에서 필드명 `"maxCapacity"`를 찾아야 하는데 `"\"maxCapacity\""`를 찾으니 항상 nil → max=0 → 항상 `QUEUE_CAPACITY_FULL`.

`StringRedisTemplate`으로 전환해서 해결했습니다.

### 수연: updateSeatEnterMeta race condition
...
매 iteration마다 새 유저를 생성하니까 setup 단계에서 504가 터졌습니다. `data=null`이 되면 초당 수십만 빈 iteration이 폭주하면서 Mimir push까지 병목이 걸렸어요.

VU당 1명 고정(`__VU` 기반 uniqueId) + setup에서 `bulk` API로 사전 생성하는 방식으로 해결했습니다.

---

## 🏃 준상: Redis Hash + Scheduler 승격
...
준상 구현체의 가장 큰 강점은 **신뢰성**이에요.

3000 VU에서도 `queue_pass_rate`가 **100%**입니다. 대기열에 들어간 사람은 전원 통과해요.
`ticket_success`도 86.89%로 3개 구현체 중 가장 높은 수준을 유지합니다.

반면 약점은 3000 VU에서의 **레이턴시 급증**이에요.
...
성전 구현체의 특징은 한마디로 **"느리지만 절대 안 깨진다"**예요.

`http_req_failed`가 **모든 VU에서 0.00%**입니다.
300 VU에서든 3000 VU에서든 에러가 단 한 건도 없어요.

API 레이턴시도 압도적입니다. 3000 VU에서 `http_req p95`가 **501ms** — 준상(3,102ms)의 1/6 수준이에요.
`queue_status p95`는 300 VU(281ms) → 3000 VU(500ms)로 **1.8배**밖에 증가하지 않습니다.

이 안정성의 비밀은 **Redis ZSET + 이벤트 기반 승격**에 있어요.
ZSET은 정렬이 O(log N)으로 빠르고, 승격을 Redis Key Expiration Event로 처리하니까 polling 요청이 서버를 직접 압박하지 않습니다.

하지만 대가가 있습니다. **iterations가 압도적으로 적어요.**
3000 VU에서 290 iterations — 준상(763)의 38%, 수연(6,574)의 4.4% 수준입니다.
이벤트 기반 승격은 비동기 특성상 슬롯 회전이 느립니다.

또 하나의 약점은 **queue_wait p95**예요. 3000 VU에서 4분 48초.
중간값(p50)은 23초로 빠르지만, 꼬리가 매우 길어요.
...
수연 POC는 300 VU 테스트를 건너뛰고 1000 VU부터 시작했습니다.
...
3000 VU 기준으로 준상(763)의 **8.6배**, 성전(290)의 **22.7배** 처리량입니다.

비결은 **동적 publishedRank 계산**이에요. 유저가 leave하면 즉시 `availableSlots`가 증가하고, 다음 status 폴링에서 바로 승격 판단이 됩니다. Scheduler를 기다릴 필요가 없어요.

3000 VU에서 active가 98~100/100으로 슬롯을 거의 100% 활용하고 있다는 것도 인상적입니다.

하지만 약점도 명확해요.

**`queue_pass_rate`가 3000 VU에서 57.90%로 떨어집니다.** 준상/성전이 100%인 것과 대비되는 수치예요.
이유는 빠른 회전 속도 때문에 동시 좌석 경합이 증가하고, AVAILABLE 좌석이 소진되면서 예매 자체가 실패하는 건데요 — 대기열 자체의 문제라기보다는 좌석 수 대비 처리량이 너무 높아서 생기는 현상입니다.

`queue_status p95`는 2.18s → 2.13s로 **3000 VU에서도 안정적**이에요.
1000 VU든 3000 VU든 pod 1개의 한계점에서 비슷한 레이턴시를 보여줍니다.
준상이 3000 VU에서 3.1s로 급증하는 것과 대조적이에요.
...
| **http_req p95** | 2.18s | 390ms | **282ms** | 성전 레이턴시 최저 |
| **http_req_failed** | 0.51% | 0.013% | **0.00%** | 성전 에러 0 |
| **queue_wait avg** | 56.4s | 67.7s | **40s** | 성전 대기 최단 |
| **queue_wait p95** | **1m17s** | 2m16s | 2m54s | 수연 꼬리 최단 |
| **queue_poll avg** | **27회** | 52.2회 | 7.3회 | 성전 폴링 최소 |

1000 VU에서는 세 구현체의 성격 차이가 명확하게 드러납니다.

수연은 iterations 5,495로 **처리량이 압도적**이에요.
준상 대비 48% 더 많은 예매를 완료했습니다.
동적 승격 방식이 leave 즉시 다음 유저를 승격시키니까 슬롯 회전이 빨라요.

준상은 **ticket_success 97.96%**로 예매 성공률이 가장 높습니다.
Scheduler가 빈 슬롯만큼만 승격하니까 동시 경합이 적어요.

성전은 **http_req p95 282ms**로 API 응답이 가장 빠릅니다.
에러도 0건. 하지만 iterations 417로 슬롯 회전이 느려요.
...
| **http_req p95** | 2.13s | 3.1s | **501ms** | 성전 압도적 |
| **http_req_failed** | 0.63% | 0.003% | **0.00%** | 성전 에러 0 |
| **queue_wait p50** | 2m19s | 53s | **23s** | 성전 중간값 최단 |
| **queue_wait p95** | 2m54s | **2m31s** | 4m48s | 준상 꼬리 최단 |
| **HTTP reqs** | **445K** | 329K | 309K | 수연 네트워크 부하 최대 |

3000 VU에서는 차이가 더 극단적으로 벌어집니다.

수연의 iterations가 6,574로 준상(763)의 **8.6배**에요.
3000명이 동시에 몰려도 슬롯을 98~100% 활용하면서 빠르게 회전합니다.

성전은 `http_req p95`가 **501ms**로 3000 VU에서도 여전히 빠릅니다.
준상이 3.1s로 급증하는 것과 대조적이에요.
300 VU(309ms) → 3000 VU(501ms)로 **1.6배밖에 안 늘어나는** 스케일링 내성이 인상적입니다.

준상은 `queue_pass_rate` 100%를 끝까지 유지합니다. 대기열에 들어간 사람은 **반드시** 통과해요.
...
하지만 pass_rate 57.90%로 안정성은 부족해요.
...
하지만 이벤트 기반 승격의 비동기 특성으로 슬롯 회전이 느려요.

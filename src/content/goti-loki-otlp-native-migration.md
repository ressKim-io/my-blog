...
> Alloy의 `otelcol.exporter.loki`에서 hint attribute가 silent fail하면서 label 승격이 안 됐다. Loki 3.x native OTLP 엔드포인트로 전환하고, 서버 측 `otlp_config`로 label을 제어하는 방식으로 해결했다.
...
Prometheus → Mimir 전환을 완료한 뒤 Log Explorer 대시보드를 열었더니 **모든 패널이 빈 결과**를 반환했어요.
...
stream selector에서 존재하지 않는 label로 필터링하니 당연히 결과가 0건이에요.
...
이 hint는 `otelcol.exporter.loki`에게 "log_type을 Loki index label로 승격해라"라고 지시하는 거예요.

문제는 **이게 작동하지 않았다**는 거예요.
에러 로그도 없었습니다. 완전한 silent fail이에요.

이건 Alloy v1.8.3~v1.13.2에서 지속되는 known issue였습니다.
[GitHub Issue #2064](https://github.com/grafana/alloy/issues/2064)와 [#3216](https://github.com/grafana/alloy/issues/3216)에서 보고된 문제예요.

`otelcol.exporter.loki`는 자체 label 제어 설정이 **없고**, 오직 hint attributes에만 의존합니다.
hint가 작동하지 않으면 label 승격 자체가 불가능해요.

---

## 🤔 대안 분석: Native OTLP vs 기존 exporter

이 시점에서 두 가지 방향이 있었어요.

### 기존 방식 유지 + 우회

`loki.process`를 사용해서 JSON body를 파싱하고 `stage.labels`로 승격하는 방법이에요.
...
Loki 3.x부터 `/otlp/v1/logs` 네이티브 OTLP 엔드포인트를 제공해요.
...
Native OTLP로 전환해도 한 가지 문제가 남아요.

Loki의 `otlp_config`에서 `index_label` action은 **resource attributes에만** 지원됩니다.
log attributes에는 structured metadata나 drop만 가능해요.

`log_type`은 원래 log attribute예요.
...
structured metadata로 저장하면 `| log_type="payment"` 같은 pipe filter로만 접근 가능해요.
`{log_type="payment"}` 같은 **stream selector로는 쓸 수 없습니다**.

이게 왜 문제냐면, retention policy에서 label selector가 필요하기 때문이에요.
...
`log_type`의 값은 `app`, `payment`, `audit` 3가지뿐이에요.
...
hint attribute에 의존하던 방식보다 훨씬 안정적이에요.
...
stream selector로 사용해야 하는 log attribute가 있다면, OTTL로 resource에 복사하는 게 현재 유일한 방법이에요.

### Grafana의 공식 방향을 따르자

`otelcol.exporter.loki`는 여전히 동작하지만, Grafana의 모든 개발 리소스가 native OTLP에 집중되고 있어요.
deprecation 공식 발표는 아직 없지만, 새 기능 추가도 없습니다.
신규 구축이라면 처음부터 native OTLP를 선택하는 게 맞아요.

...
Server를 먼저 올리고 Monitoring을 나중에 올리면, Monitoring의 네트워크가 아직 없어서 실패할 수 있습니다.
`name:` + `driver:` 조합이면 없을 때 자동으로 생성해주니까 순서에 의존하지 않을 거라 생각했습니다.

로컬에서는 정상 동작했습니다. 하지만 EC2에서는 달랐습니다.
...
수동으로 만든 네트워크에는 compose 라벨(`com.docker.compose.network`)이 없습니다.
...
`docker network create`로 수동 생성한 네트워크는 "남의 것"으로 취급합니다.

반면 `external: true`는 라벨을 체크하지 않습니다.
"이미 존재하는 외부 네트워크를 그대로 쓰겠다"는 명시적 선언이기 때문입니다.
...
이전 배포(PR #32, #34)에서도 동일한 증상이 있었습니다.
...
이상한 점은, Docker 컨테이너 자체의 healthcheck에서는 **healthy**로 표시된다는 것입니다.
...
Loki의 `/ready` 엔드포인트는 GET 요청에는 응답하지만, **HEAD 메서드를 지원하지 않습니다**.
HEAD 요청을 받으면 405(Method Not Allowed)나 빈 응답을 반환합니다.

다른 서비스(Prometheus, Grafana 등)는 HEAD 요청도 처리하기 때문에 문제가 없었던 것입니다.
...
실질적으로 "URL이 응답하는지"만 확인하는 용도로는 이 방식이 더 안전합니다.
...
`name:` + `driver:` 조합은 compose가 네트워크 생명주기를 완전히 관리하는 경우에만 사용해야 합니다.
...
healthcheck에서는 `wget -qO /dev/null`이 더 범용적입니다.

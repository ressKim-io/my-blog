...
> EC2 모니터링 대시보드 14개를 체계적으로 검증했더니 5건의 No Data 문제가 나왔다. PromQL 폴백, 히스토그램 버킷, Loki 라벨 승격, 메트릭명, 크로스 데이터소스 변수 — 원인이 전부 달랐다.
...
앞선 글들에서 OTel 레이블 매핑 이슈를 해결한 뒤, EC2 환경에서 14개 Grafana 대시보드를 SSM으로 접속해 하나씩 검증했어요.
...
5xx 에러가 한 건도 없는 정상 상태에서 `goti:sli:availability:5m` 등 5개 recording rule이 No Data를 반환했어요.
...
현재는 `or vector(0)` 폴백이 있어서 동작해요.
5xx가 없으면 `sum(rate({5xx}))` = empty → `or vector(0)` = 0 → 정상 계산.

하지만 문제는 **향후 MSA 전환** 시에요.
...
`or vector(0)` → `or on() vector(0)`로 변경했어요.
...
5개 availability rule 모두 수정했어요.

`on()` 없이 `or vector(0)`를 쓰면 단일 서비스에서는 동작하지만, 그루핑이 추가되면 깨진다. 처음부터 `on()`을 쓰는 게 안전하다.
...
`goti:apdex:score` recording rule이 No Data를 반환했어요.

### 🤔 원인

Apdex 계산에서 `le="2.0"` 버킷을 사용하고 있었습니다.

문제는 OTel 기본 히스토그램 버킷에 `2.0`이 없다는 거예요.
...
Micrometer 시절에는 커스텀 버킷을 사용해서 `2.0`이 있었지만, OTel로 전환하면서 기본 버킷을 쓰게 된 거예요.
...
Apdex 임계값이 2.0초에서 2.5초로 약간 느슨해졌지만, 현실적으로 2.0~2.5초 구간의 차이가 Apdex 점수에 미치는 영향은 미미해요.

**OTel 히스토그램 버킷 목록을 확인하지 않고 PromQL을 작성하면 이런 문제가 생긴다.**
...
Alloy transform processor에서 `loki.attribute.labels` 힌트로 `log_type` 라벨을 승격시키려 했는데, Loki에 라벨이 생성되지 않았어요.
...
Alloy 컨테이너는 config 변경 시 자동 reload되지 않아서, 컨테이너를 재시작해야 했어요.
...
JVM Deep Dive 대시보드에서 HikariCP 커넥션 풀 패널이 No Data였어요.
...
OTel Java agent가 레거시 `_milliseconds` 접미사를 사용하고 있었어요.
새로운 semantic conventions(`_seconds`)는 opt-in이 필요합니다.

또한 `db_client_connections_timeouts_total`이라는 메트릭은 아예 존재하지 않았어요.
...
하나의 대시보드에서 Prometheus, Loki, Tempo 세 데이터소스를 동시에 사용하는데, 서비스 이름 체계가 달랐어요.
...
OTel → Prometheus에서는 `job = <namespace>/<service.name>` 형태지만, Loki와 Tempo는 `service.name` 값을 그대로 사용해요.

데이터소스마다 서비스 이름 체계가 다른 것이 근본 원인이다.
...
원인은 전부 달랐지만, 공통점이 있어요.

**Micrometer에서 OTel로 전환하면서 "당연히 같을 것"이라고 가정한 것들이 달랐다.**
...
OTel 전환 후 대시보드를 마이그레이션할 때 확인해야 할 항목들이에요.
...
이 시리즈를 통해 OTel 전환의 레이블 이슈를 총정리했다. SDK 버전 충돌부터 크로스 데이터소스 변수까지, 결국 **실측 검증 없이는 안전한 마이그레이션이 불가능하다**는 게 핵심 교훈이다.

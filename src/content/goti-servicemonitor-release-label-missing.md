...
> kube-prometheus-stack의 Prometheus가 `release: kube-prometheus-stack-dev` 라벨이 있는 ServiceMonitor만 인식한다. Alloy/Loki/Tempo는 별도 chart라 이 라벨이 없어서 scrape 대상에 등록되지 않았다.
...
Monitoring Stack Health 대시보드에서 Alloy 메모리/CPU, Loki Ingestion Rate, Tempo Spans 패널이 전부 비어있었어요.
...
Pod는 전부 Running, 2/2 Ready였어요. 문제는 **Prometheus가 ServiceMonitor를 인식하지 못하는 것**이었습니다.
...
kube-prometheus-stack의 Prometheus CR이 `serviceMonitorSelector`를 설정하면, **이 라벨이 있는 ServiceMonitor만** scrape 대상으로 등록해요.

Alloy, Loki, Tempo는 별도 Helm chart로 배포되니까 이 라벨이 자동 부여되지 않습니다.
ServiceMonitor는 존재하지만 Prometheus가 무시하고 있었던 거예요.
...
각 chart의 values에 라벨을 추가했어요. chart마다 키 이름이 다릅니다:
...
`additionalLabels` vs `labels` vs `monitoring.serviceMonitor.labels` — chart마다 달라서 `helm show values`로 사전 확인이 필요해요.

### Kind 전용: scrape 불가 컴포넌트 비활성화

etcd, kube-proxy, kube-scheduler, kube-controller-manager는 Kind에서 **localhost에만 바인딩**되어 외부 scrape가 불가능해요.
...
kube-prometheus-stack의 `serviceMonitorSelector`가 특정 라벨을 요구하면, 모든 외부 chart의 ServiceMonitor에 이 라벨을 명시적으로 추가해야 해요.
...
Kind에서 control plane 컴포넌트는 localhost 바인딩이라 Prometheus가 scrape할 수 없어요.
이건 Kind의 알려진 제한이고, 이 컴포넌트들의 ServiceMonitor를 비활성화하는 게 정답이에요.

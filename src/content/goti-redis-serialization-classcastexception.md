...
`LinkedHashMap`을 `ReservationSessionCache`로 캐스팅할 수 없다고요?
분명히 `ReservationSessionCache` 객체를 Redis에 넣었는데, 꺼내니까 `LinkedHashMap`이 나온다.
...
`value`의 실제 타입이 `LinkedHashMap`이면, `ReservationSessionCache`로 캐스팅이 당연히 실패해요.
...
`GenericJackson2JsonRedisSerializer`는 이름에 "Generic"이 붙어있어요.
**모든 타입을 JSON으로 직렬화**할 수 있다는 뜻입니다.

그런데 역직렬화할 때 문제가 생깁니다.
JSON `{"sessionId":"abc","seatId":42}`만 보고는 이게 어떤 클래스인지 알 수 없어요.
...
그러면 간단하잖아요. `activateDefaultTyping` 넣으면 끝 아닌가?

문제는 **기존 Redis 데이터**입니다.

이미 prod Redis에는 `@class` 필드 없이 저장된 데이터가 수천 건 있어요.
...
TTL이 만료될 때까지 기다리는 것도 방법이지만, TTL이 긴 캐시도 있어서 완전 전환까지 시간이 걸려요.

**그래서 이 방법은 기각했습니다.**

### 수정: convertValue fallback

대신 `RedisCache.get()`에 안전한 변환 로직을 추가했어요.
...
이 메서드는 Java 객체를 다른 Java 객체로 변환해요.
...
수연 대기열 POC 부하테스트를 돌리는데, 이번엔 **API 응답 자체가 깨졌어요**.
...
여기에 우리가 `@Bean ObjectMapper`를 하나 더 등록하면 **충돌**이 발생해요.
...
이 ObjectMapper에는 Spring Boot가 자동으로 등록하는 여러 Module이 빠져있어요.
`ApiSuccessResponse`처럼 커스텀 직렬화가 필요한 응답 객체에서 에러가 터집니다.

**한 마디로, Redis용 ObjectMapper가 HTTP 응답 직렬화까지 오염시킨 것이다.**
...
이렇게 하면 이 ObjectMapper는 Spring IoC 컨테이너에 등록되지 않아요.
...
어떤 타입이든 넣을 수 있다는 건 편리하지만, 꺼낼 때 타입을 보장할 수 없어요.
컴파일러가 잡아줄 수 없는 런타임 에러의 원인이 됩니다.

둘째, `GenericJackson2JsonRedisSerializer`는 만능 직렬화기처럼 보이지만 함정이 있어요.
`activateDefaultTyping`이 없으면 역직렬화 결과가 `LinkedHashMap`이 되고, 있으면 JSON에 FQCN(Fully Qualified Class Name)이 박힙니다.
FQCN이 박히면 패키지 이동이나 클래스 이름 변경 시 기존 Redis 데이터가 전부 깨져요.
...
지금은 `convertValue` fallback으로 방어하고, 점진적으로 도메인별 Repository 패턴으로 전환할 계획이에요.
...
수연 대기열 POC 부하테스트가 진행 중이었기 때문에, develop 브랜치를 거치지 않고 바로 배포했어요.
...
- **`GenericJackson2JsonRedisSerializer`에 `activateDefaultTyping`이 없으면**, 역직렬화 결과는 원본 타입이 아니라 `LinkedHashMap`이다. 특히 Java record 타입은 100% fallback된다.
- **`@Bean ObjectMapper`를 함부로 등록하면 안 된다.** Spring Boot의 자동 설정 ObjectMapper와 충돌해서, Redis뿐 아니라 HTTP 응답 직렬화까지 오염시킬 수 있다.
- **`RedisTemplate<String, Object>` 조합 자체가 구조적 위험**을 갖고 있다. 장기적으로는 `RedisTemplate<String, String>` + 도메인별 Repository 패턴이 안전하다.
- **prod 환경 핫픽스는 반드시 동기화 추적**이 필요하다. deploy/dev에 직접 넣은 커밋은 develop cherry-pick을 잊지 말 것.

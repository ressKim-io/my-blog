...
> ServiceMonitor를 만들었는데 Grafana가 텅 비어있다. Istio AuthorizationPolicy, ingress/egress NetworkPolicy 3곳에서 포트 9090이 누락되어 있었고, DB 쿼리는 잘못된 스키마를 참조하고 있었다.
...
goti-metrics-collector를 배포하고 ServiceMonitor를 생성했는데, Grafana에서 **모든 패널이 No data**였어요.
...
`up=0`이면 Alloy가 scrape 자체에 실패했다는 뜻이에요.
...
goti namespace는 defense-in-depth 구조예요.
Istio AuthorizationPolicy(L7) + NetworkPolicy(L3/L4) 양방향으로 보호됩니다.

기존에 허용된 포트는 Spring Boot용이었어요:
...
scrape가 성공한 뒤 `/metrics`를 확인했는데, 비즈니스 메트릭이 없었어요.
...
collector가 SQL에서 스키마를 명시하지 않고 있었어요.
...
데이터는 `ticketing_service` 스키마에 있었어요.

### 가설 3: 잘못된 테이블 참조

collector가 `game_seat_inventories` 테이블을 집계 소스로 사용하고 있었는데, 이 테이블은 **양 스키마 모두 0건**이었어요.
실제 좌석 데이터는 `seat_statuses` + `seats` + `seat_sections` + `seat_grades` JOIN으로 집계해야 합니다.

### 가설 4: aggregator 미연결

파생 지표를 계산하는 `aggregator.UpdateSold()`, `aggregator.UpdatePayment()` 메서드가 존재하지만, poller에서 **호출하지 않고 있었어요**.
...
이번 건의 핵심 교훈이에요. 기존 Spring Boot(8080)에 최적화된 보안 정책이 있는 상태에서, 새 서비스가 다른 포트(9090)를 쓰면 **3곳 모두 업데이트**해야 합니다.
...
한 곳만 빠져도 연결이 안 되고, 디버깅 시 "어느 레이어가 차단하는지" 파악이 어려워요.
위에서부터 하나씩 열면서 확인하는 게 가장 확실합니다.

### PostgreSQL search_path를 신뢰하지 말자

외부 서비스가 DB에 직접 접근할 때는 **항상 스키마를 명시**해야 해요.
`search_path`는 세션 설정이라 예상과 다를 수 있고, 같은 테이블 이름이 여러 스키마에 존재할 수 있습니다.

### Prometheus GaugeVec은 Set() 호출 전까지 메트릭을 노출하지 않는다

`client_golang`의 `GaugeVec`은 한 번도 `Set()`이 호출되지 않은 label 조합은 `/metrics`에 **아예 나타나지 않아요**.
DB에 해당하는 row가 0건이면 메트릭 자체가 사라지는 거예요.
이건 "값이 0"이 아니라 "메트릭이 없음"으로 처리되니까, Grafana에서 No data로 보입니다.

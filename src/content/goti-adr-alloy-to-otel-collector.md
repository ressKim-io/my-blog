...
![기존 Alloy 기반 관측성 파이프라인 (Before)](/diagrams/goti-adr-alloy-to-otel-collector-1.svg)
...
**Before: Alloy 기반** — 위 "현재 아키텍처" 다이어그램 참조 (logs만 Kafka 버퍼 누락, 30만 동시 접속 스파이크 시 OOM 위험)

**After: OTel Collector + Kafka 전면 버퍼링**

![OTel Collector + Kafka 전면 버퍼링 (After)](/diagrams/goti-adr-alloy-to-otel-collector-2.svg)

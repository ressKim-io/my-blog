...
> Grafana Alloy의 River 문법과 제한된 커뮤니티 자료 때문에 CNCF 표준 OTel Collector로 전환했다. Kafka 버퍼링을 logs/traces 전체로 확장해 30만 동시 접속 스파이크에 대비한다.
...
Goti 프로젝트의 관측성 파이프라인은 이렇게 구성되어 있었어요.
...
핵심 문제는 **logs가 Kafka 없이 Loki로 직접 전송**된다는 점이에요.
Traces만 Kafka로 버퍼링하고 있었습니다.

### 문제점 4가지

이 구조에서 운영하면서 4가지 문제를 느꼈어요.

**1. 커뮤니티 자료 부족**

Alloy는 River라는 독자적인 문법을 사용해요.
Kafka 연동에서 문제가 생기면, 검색해도 OTel Collector YAML 기반 자료만 나옵니다.
River 문법으로 변환하는 건 결국 팀원이 직접 해야 했어요.

**2. Kafka 연동 자료 부족**

`otelcol.exporter.kafka` 블록의 세부 설정을 찾기 어려웠습니다.
OTel Collector contrib의 `kafkaexporter`는 문서화가 잘 되어 있지만, Alloy에서 래핑한 버전은 예제가 부족해요.

**3. 스파이크 대응 불완전**

30만 동시 접속 목표인 Goti에서, 초기 폭주 시 logs가 Loki로 직접 쏟아지면 OOM 위험이 있었어요.
Traces는 이미 Kafka로 버퍼링하고 있는데, logs만 빠져 있는 구조였습니다.

**결국 logs도 Kafka 버퍼링이 필요했다.**

**4. 벤더 종속**

Alloy에는 Grafana 전용 블록이 꽤 있어요.
...
OTel Collector가 커뮤니티, Kafka 연동, 표준 YAML 면에서 압도적이었어요.
유일한 단점은 Alloy의 파이프라인 시각화 UI를 잃는 것인데, Grafana 대시보드로 대체할 수 있었습니다.

### 메트릭 저장소: Mimir 분산 유지

"활성 시리즈가 약 50만인데 분산 모드가 과잉 아닌가?"라는 의문이 있었어요.
...
공식 `mimir-distributed` Helm chart에 SingleBinary 모드가 없어요 (Grafana Discussion #6211).
SimpleScalable 모드도 Mimir 3.0에서 제거되었습니다 (PR #12584).
커스텀 StatefulSet을 직접 만드는 것보다 공식 chart를 유지하는 게 운영 부담이 적다고 판단했어요.

**분산 모드가 과잉인 건 맞지만, 공식 chart의 한계 때문에 현상 유지를 선택한 것이다.**
...
백엔드팀이 이미 Strimzi Kafka를 운영하고 있었어요.
Traces 버퍼링도 Kafka topic으로 하고 있었고요.

추가 인프라 비용 0으로 logs topic만 추가하면 되는 Kafka가 가장 현실적이었습니다.

30만 스파이크 시 초당 수만 건의 로그가 쏟아져도, Kafka의 디스크 기반 버퍼링은 OOM 걱정이 없어요.
...
**Metrics**: OTel Collector Front에서 `prometheusremotewrite` exporter로 Mimir에 직접 전송해요.
Kafka를 거치지 않는 이유는, 메트릭은 실시간성이 중요하고 볼륨도 상대적으로 적기 때문입니다.

**Logs**: 기존에 Loki 직접 전송이던 것을 Kafka topic(`observability.logs.v1`)으로 전환했어요.
Collector Back이 Kafka에서 소비해 Loki OTLP로 전달합니다.

**Traces**: 기존과 동일하게 Kafka 버퍼링을 유지해요.
...
`kafkaexporter`/`kafkareceiver`가 공식 contrib으로 문서화가 잘 되어 있어요.
팀원이 트러블슈팅할 때 검색하면 바로 답을 찾을 수 있습니다.

Alloy의 `otelcol.*` 블록이 OTel Collector의 컴포넌트와 **개념적으로 대응**하기 때문에 어떤 컴포넌트가 필요한지는 동일합니다. 하지만 **파이프라인 배선 구문은 완전히 다르기** 때문에 마이그레이션 시 YAML 재작성이 필요해요.
...
반면 Alloy는 River 문법으로 output 블록을 명시적으로 연결해요:
...
**전환 리스크가 낮다고 오해하면 안 된다.** YAML 재작성은 필수입니다.

**2. Mimir 분산 유지: 공식 chart 한계**

공식 `mimir-distributed` Helm chart에 SingleBinary 모드가 없어요 (Discussion #6211).
SimpleScalable도 Mimir 3.0에서 제거되었습니다.
커스텀 StatefulSet을 만드는 것보다 공식 chart를 유지하는 게 장기적으로 안전합니다.

**3. Kafka 공유: 추가 비용 0**

백엔드팀이 이미 Strimzi Kafka를 운영하고 있고, traces도 이미 Kafka 버퍼링 중이에요.
...
**Alloy Web UI 상실**: Alloy는 `:12345` 포트에서 파이프라인 DAG를 시각적으로 보여줬어요.
OTel Collector에는 이 기능이 없습니다.
대신 Grafana 대시보드(`infra-otel-pipeline.json`)로 파이프라인 상태를 모니터링할 계획이에요.

**mimir.rules.kubernetes 자동 sync 상실**: Alloy가 Kubernetes의 PrometheusRule CR을 감시해서 Mimir ruler에 자동으로 sync하던 기능이에요.
OTel Collector에는 이 기능이 없으므로, `mimirtool` CronJob 또는 수동 sync로 대체해야 합니다.

**Alloy 클러스터링 상실**: DaemonSet 간 타겟 분배 기능이에요.
...
**도구보다 생태계를 보자.** Alloy 자체는 훌륭한 도구예요.
하지만 River 문법이라는 독자적 생태계가 팀의 학습과 트러블슈팅을 느리게 만들었습니다.
CNCF 표준(OTel Collector YAML)은 검색 한 번에 답이 나와요.

**버퍼링은 전부 아니면 전무.** Traces만 Kafka로 버퍼링하고 logs는 직접 전송하는 반쪽짜리 구조는 위험했어요.
30만 스파이크에서 logs가 Loki를 OOM시키면, traces만 살아남아도 의미가 없습니다.

**공식 chart의 한계도 수용해야 한다.** Mimir 싱글바이너리가 이상적이지만, 공식 Helm chart가 지원하지 않으면 무리하게 커스텀하지 않는 게 장기적으로 낫다.

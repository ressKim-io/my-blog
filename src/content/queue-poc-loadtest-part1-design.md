...
사람별 헬퍼 3개 + 시나리오 3개 = 6파일. 공통 인터페이스는 `enterQueue() → waitForAdmission() → enterSeat() → [ticketing] → leaveQueue()` 흐름으로 설계했습니다.
...
`POST /api/v1/queue/junsang/validate`가 **401(empty body)** 로 떨어졌습니다. 모든 queue API가 401, 응답 body는 비어 있었습니다. 뭐지?

...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Rolling Update          Canary                 A/B Testing   │
│   ════════════════        ════════════════       ════════════  │
│                                                                 │
│   v1 → v2 순차 교체       v1 90% + v2 10%        조건별 분기    │
│                           점진적 증가                           │
│                                                                 │
│   ┌───┬───┬───┬───┐       ┌───────────────┐     ┌────────────┐ │
│   │v1 │v1 │v1 │v1 │       │ v1  90%       │     │ 헤더=A → v1│ │
│   └───┴───┴───┴───┘       ├───────────────┤     │ 헤더=B → v2│ │
│          ↓                │ v2  10%       │     └────────────┘ │
│   ┌───┬───┬───┬───┐       └───────────────┘                    │
│   │v2 │v1 │v1 │v1 │                                            │
...
│   └───┴───┴───┴───┘                                            │
│                                                                 │
│   Kubernetes 기본         Istio VirtualService   Istio match   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│              Rolling Update 트래픽 분배 문제                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   시간 T=0 (시작)                                               │
│   Pod: [v1] [v1] [v1] [v1]     트래픽: 100% v1                  │
│                                                                 │
│   시간 T=1 (1개 교체)                                           │
│   Pod: [v2] [v1] [v1] [v1]     트래픽: 25% v2, 75% v1           │
│                                                                 │
│   시간 T=2 (2개 교체)                                           │
│   Pod: [v2] [v2] [v1] [v1]     트래픽: 50% v2, 50% v1           │
│                                                                 │
│   ⚠️ 문제점:                                                    │
│   - 트래픽 비율 = Pod 비율 (세밀한 제어 불가)                   │
│   - 10% 트래픽만 v2로? → Pod 10개 중 1개 = 비효율               │
│   - 문제 발생 시 빠른 롤백 어려움                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│      ┌──────────────────────────────────────────┐               │
│      │ 0분:  v1 100% │ v2 0%                    │               │
│      │ 1분:  v1 90%  │ v2 10%  ← 메트릭 체크   │               │
│      │ 2분:  v1 80%  │ v2 20%  ← 메트릭 OK     │               │
│      │ 3분:  v1 70%  │ v2 30%  ← 메트릭 OK     │               │
│      │ ...                                      │               │
│      │ 5분:  v1 50%  │ v2 50%  ← 최대 도달     │               │
│      └──────────────────────────────────────────┘               │
│                                                                 │
│   3. 메트릭 실패 시 자동 롤백                                   │
│      error rate > 1% → 즉시 v1 100%                             │
│                                                                 │
│   4. 성공 시 프로모션                                           │
│      v2 → Primary로 승격                                        │
│                                                                 │
...
│                                 - 매출 비교                     │
│                                                                 │
│   같은 사용자가 v1/v2 왔다갔다  같은 사용자는 항상 같은 버전    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│   ═══════════════════           ═══════════════════             │
│                                                                 │
│   안전한 점진적 배포            Canary (weight)                 │
│   새 버전 안정성 검증           - 5% → 25% → 50% → 100%         │
│                                                                 │
│   기능 효과 비교                A/B Testing (match)             │
│   UX/전환율 측정                - 헤더/쿠키 기반 분기           │
│                                                                 │
│   무영향 사전 테스트            Dark Launch (mirror)            │
│   성능/오류 사전 검증           - 실제 트래픽 복제              │
│                                                                 │
│   내부 테스터 우선 접근         Feature Flag (match + weight)   │
│   베타 테스트                   - 특정 조건 + 나머지 Canary     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

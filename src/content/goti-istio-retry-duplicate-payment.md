...
K6로 동시 사용자 시나리오를 실행했는데, 결과가 이상했습니다.
...
첫 번째 요청은 정상적으로 시작되었고, 두 번째와 세 번째는 "결제 가능한 주문 상태가 아닙니다"라는 경고와 함께 실패했습니다.

뭐지? K6 스크립트가 중복 요청을 보내고 있나?

K6 코드를 확인했습니다. 결제 요청은 한 번만 보내고 있었습니다.

그렇다면 누가 같은 요청을 3번 보낸 걸까?

시간 간격을 보면 힌트가 있습니다. `538ms → 717ms → 804ms`로 179ms, 87ms 간격으로 거의 즉시 재요청이 들어오고 있었습니다.
이것은 사람이 아니라 **인프라 레이어에서 자동으로 재시도**하고 있다는 뜻입니다.
...
payment 서비스가 500을 반환하면, Istio가 자동으로 재시도합니다.

### `attempts: 2`의 의미

Istio의 `attempts` 필드는 직관적이지 않습니다.

많은 사람이 "총 2번 시도"라고 생각하지만, 실제로는 **`attempts: 2` = 원본 요청 1회 + 재시도 2회 = 총 3회**입니다.
...
첫 번째 시도에서 500이 반환되자, Istio가 179ms 후에 즉시 retry를 보냈습니다.
...
HTTP를 통해 외부 서비스의 상태를 변경하는 것입니다.
...
payment의 `initPayment()` 메서드가 `@Transactional`로 감싸져 있습니다.
이 트랜잭션 안에서 ticketing 서비스에 `confirmPayment()`를 호출합니다.

ticketing 서비스는 주문 상태를 `PENDING`에서 `PAID`로 변경합니다.
이 변경은 **ticketing 서비스의 자체 트랜잭션**으로 즉시 커밋됩니다.

그 다음 `orderDetail()` 호출에서 실패가 발생합니다.
이때 payment의 `@Transactional`이 롤백되지만, 이미 HTTP로 보낸 ticketing 요청은 되돌릴 수 없습니다.

결과적으로 payment는 500을 반환합니다.

**2단계: Istio retry — 이미 변경된 상태에서 재시도**

Istio는 500을 보고 "서버 오류니까 다시 시도하자"라고 판단합니다.

하지만 ticketing 서비스에서 주문 상태는 이미 `PAID`입니다.
payment가 다시 `confirmPayment()`를 호출하면, ticketing은 "이미 결제된 주문"이라며 400을 반환합니다.
...
세 번째 시도도 동일하게 실패합니다.
Istio는 모든 retry를 소진하고 클라이언트에 500을 반환합니다.
...
- **ticketing 서비스**: 이미 PAID로 변경된 주문 상태는 그대로입니다. 되돌릴 방법이 없습니다.

이 상태에서 retry가 들어오면 ticketing은 "이미 PAID인데 왜 또 결제하려고 해?"라며 400을 반환하는 것입니다.

### 왜 "일부 성공 + 전체 롤백"이 위험한지

이 패턴이 특히 위험한 이유를 정리해보겠습니다.
...
GET 요청은 멱등합니다. 10번 호출해도 결과가 같습니다.
하지만 POST로 결제를 처리하는 API는 비멱등합니다.
한 번 호출로 상태가 변경되면, 같은 요청을 다시 보내도 원래 결과를 재현할 수 없습니다.

**이것이 Istio retry + 비멱등 API 조합이 치명적인 이유입니다.**
...
변경은 단 한 줄입니다. `5xx`를 `retryOn`에서 제거했습니다.

### 왜 reset, connect-failure는 안전한가

`retryOn`에 남겨둔 두 가지 조건을 살펴보겠습니다.
...
이 두 경우는 **서버가 요청을 받기 전에 실패**한 상황입니다.
...
서버가 요청을 받았다는 건, **이미 부분적으로 처리했을 수 있다**는 뜻입니다.
결제 API처럼 외부 서비스를 호출하는 경우, 5xx 시점에 이미 외부 상태가 변경되었을 수 있습니다.

이것이 `5xx` retry가 비멱등 API에서 위험한 근본적인 이유입니다.
...
`@Transactional`이 롤백되면서 3번의 DB 저장도 취소됩니다.

**하지만 1번의 변경은 되돌릴 수 없습니다.**

이것이 `@Transactional` 안에서 외부 API를 호출하면 안 되는 이유입니다.
...
`@Transactional` 내에서 두 단계로 나눕니다.

1. 결제 정보를 DB에 저장합니다.
2. Outbox 테이블에 이벤트를 저장하고 트랜잭션을 커밋합니다.

별도 프로세스가 다음을 처리합니다.

3. Outbox 이벤트를 발행해 ticketing 서비스를 호출합니다.
4. 실패하면 재시도합니다(Outbox에 기록이 있으므로 안전합니다).

외부 서비스 호출을 트랜잭션 밖으로 빼는 것입니다.
트랜잭션이 성공한 후에만 외부 호출이 실행됩니다.

**방법 2: SAGA 패턴**

1. payment가 결제를 PENDING 상태로 시작합니다.
2. ticketing이 주문을 확인하고 성공/실패를 반환합니다.
3. 성공하면 payment 상태를 COMPLETED로 갱신합니다.
4. 실패하면 보상 트랜잭션을 실행해 ticketing을 롤백합니다.

각 서비스가 독립적으로 트랜잭션을 관리하고, 실패 시 보상 트랜잭션으로 일관성을 맞추는 패턴입니다.

두 패턴 모두 구현 복잡도가 높습니다.
당장은 Istio retry 설정 수정으로 충분하지만, 서비스가 복잡해지면 반드시 고려해야 할 패턴입니다.

### Istio retry 설정 가이드

이번 경험을 바탕으로 정리한 Istio retry 설정 가이드입니다.
...
이 테이블에서 주목할 것은 **POST (멱등키 있음)** 행입니다.

비멱등 API라도 멱등키(Idempotency Key)를 도입하면 안전하게 retry할 수 있습니다.
클라이언트가 고유한 키를 요청에 포함하고, 서버가 이 키로 중복 요청을 식별하는 방식입니다.
...
서버는 같은 `Idempotency-Key`를 가진 첫 요청에 대해서만 결제를 처리하고 결과를 저장합니다. 같은 키의 두 번째 요청부터는 저장된 결과를 그대로 반환하고, 재처리하지 않습니다.

장기적으로 결제 API에 멱등키를 도입하면, Istio retry의 `5xx`를 다시 켤 수 있습니다.
그때까지는 `reset,connect-failure`만 사용하는 게 안전합니다.

### 기존 서비스 영향 확인

이번 수정은 POC payment 서비스에만 적용했습니다.
추가로 확인이 필요한 사항들:

- **goti-payment-prod**: 동일한 VirtualService 설정을 사용하는지 확인 필요합니다. 별도 구조라 파일이 다를 수 있습니다.
- **ticketing / queue 서비스**: GET 중심의 API라 `5xx` retry를 유지해도 안전합니다. 상태를 변경하지 않기 때문입니다.
...
- **Istio `retryOn: 5xx`는 비멱등 POST API에 적용하면 안 됩니다.** 서버가 요청을 부분적으로 처리한 후 실패하면, retry가 중복 부작용을 일으킵니다.
- **`attempts: 2`는 "총 2번"이 아니라 "원본 1 + retry 2 = 총 3번"입니다.** Istio 문서를 꼼꼼히 읽어야 합니다.
- **`@Transactional` 안에서 외부 HTTP 호출은 위험합니다.** 로컬 롤백은 외부 서비스의 상태 변경을 되돌리지 못합니다.
- **`reset`과 `connect-failure`는 안전한 retry 조건입니다.** 서버에 요청이 도달하지 않았으므로 부작용이 없습니다.
- **장기적으로 멱등키(Idempotency Key) 도입을 검토해야 합니다.** 비멱등 API를 멱등하게 만들면, 네트워크 레벨의 retry를 안전하게 활용할 수 있습니다.

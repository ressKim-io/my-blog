...
모든 goti 서비스에서 새 Pod가 `ImagePullBackOff` 상태로 복구되지 않았어요.
...
ReplicaSet template을 diff해봤더니 **toleration 추가가 유일한 차이**였어요.
...
Helm values, chart template, 레포 전체를 grep했는데 **소스에 없었어요**.
...
prod 작업(spot 노드 toleration) 중에 dev 클러스터에도 kubectl로 toleration을 직접 추가했어요.

이게 연쇄 장애를 만든 과정을 따라가봅시다.
...
핵심은 **kubectl 직접 수정이 불필요한 rolling update를 트리거**했다는 거예요.
...
toleration을 제거하면 template이 원래대로 돌아가면서 BackOff Pod가 자동 정리돼요.
...
kubectl 직접 수정은 "빠른 테스트"처럼 보여요.
하지만 GitOps 환경에서는 **drift → 예측 불가능한 부작용**을 만듭니다.

이번 경우처럼 template 변경이 rolling update를 트리거하고, 그 타이밍에 ECR credential이 만료되면 전 서비스 장애가 발생해요.
...
ECR credential이 갱신돼도 kubelet의 backoff timer가 이미 수십 분 단위로 올라가 있어요.
자동 복구를 기다리는 것보다 **Pod를 삭제하고 재생성**하는 게 가장 빠른 복구 방법입니다.

### ECR credential 타이밍

ECR 토큰은 12시간 유효하고, CronJob이 6시간마다 갱신해요.
갱신 직전 타이밍에 새 Pod가 생성되면 pull에 실패할 수 있습니다.
이런 엣지 케이스를 줄이려면 갱신 주기를 더 짧게(4시간) 가져가거나, credential helper 방식을 검토해야 해요.

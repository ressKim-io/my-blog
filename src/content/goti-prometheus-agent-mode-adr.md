...
> Go cutover 후 노드를 12대에서 8대로 줄이는 과정에서 Prometheus가 단일 노드 메모리를 98%까지 점유했습니다. 근본 원인은 이미 Mimir가 모든 쿼리를 담당하고 있음에도 Prometheus가 로컬 TSDB를 유지하며 이중 저장을 지속한 구조적 문제였습니다. Agent Mode 전환으로 이중 저장을 제거하고 메모리를 70% 절감했습니다.

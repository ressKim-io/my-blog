...
> CDN 경유 테스트에서 모든 인증 API가 401. Istio RequestAuthentication에 하드코딩된 JWKS의 RSA public key가 실제 User 서비스 키와 달랐다. 19개 서비스 전부 오래된 키를 사용 중이었고, jwksUri 동적 조회도 STRICT mTLS에서 불가능했다.
...
수연님 대기열 구현체(queue-suyeon)를 Cloudflare CDN 경유로 테스트하려고 했어요.
...
오늘은 CDN을 거치는 실제 경로로 테스트하려는 거였어요.
...
**4ms는 Istio Envoy sidecar가 요청을 앱까지 보내지도 않고 차단했다는 뜻이다.**

content-type이 `text/plain`인 것도 힌트입니다.
Spring은 보통 `application/json`으로 응답해요.
...
`kid`, `iss`, `alg` 전부 정상이에요.
만료 시간도 충분히 남아있었습니다.

JWT 자체는 문제가 없다.
그렇다면 **검증하는 쪽**에 문제가 있다는 뜻이다.
...
k8s-troubleshooter로 pod 로그를 조사했더니 수상한 로그가 있었어요.
...
POC 이미지(`poc-suyeon-37-8856b54`)에 `QueueSecurityConfig`가 포함되지 않았어요.

하지만 이건 **Spring 레벨**의 문제입니다.
401 응답이 4ms만에, `text/plain`으로 왔다는 건 **Istio 레벨**에서 차단됐다는 뜻이에요.

Spring SecurityConfig 누락은 별도로 수정해야 할 문제지만, **지금 401의 원인은 아니다.**

### 가설 3: JWKS public key 불일치 → ROOT CAUSE

Istio가 JWT를 검증하려면 **공개 키(JWKS)**가 필요합니다.
RequestAuthentication에 하드코딩된 JWKS와 User 서비스의 실제 키를 비교해봤어요.
...
아! User 서비스가 JWT를 서명할 때 쓰는 private key에 대응하는 public key와, Istio가 검증에 쓰는 public key가 다른 거였어요.
...
prod-gcp 5개도 마찬가지였어요.
**총 19개 서비스가 전부 오래된 JWKS를 사용하고 있었습니다.**

이건 단순히 "하나 빠뜨렸다"가 아니라, **JWKS를 하드코딩해서 배포하는 구조 자체의 문제**다.
...
이때 JWKS의 public key가 실제 signing key와 다르면, 서명 검증이 실패하고 요청이 앱까지 도달하지 못해요.
...
1. **RequestAuthentication**: JWT를 파싱하고 서명을 검증한다 (JWKS 사용)
2. **AuthorizationPolicy**: `requestPrincipals`을 체크하여 JWT가 없거나 유효하지 않은 요청을 **차단**한다

핵심은 이겁니다.
**RequestAuthentication만 있고 AuthorizationPolicy가 없으면, JWT 검증 실패 시 요청이 그냥 통과합니다.**

queue-suyeon에는 `require-jwt` AuthorizationPolicy가 **누락**되어 있었어요.
...
**어제도 Istio JWT 검증은 실패했지만, 앱이 fallback으로 동작한 것이다.**

### 그러면 오늘은 왜 실패했나?

CDN 경유 테스트를 위해 queue-suyeon에 AuthorizationPolicy를 추가했기 때문입니다.
보안 설정을 강화하면서 `require-jwt` 정책을 넣었는데, 정작 JWKS가 틀려있었으니 모든 요청이 차단된 거예요.

**보안을 강화했더니 숨어있던 설정 오류가 드러난 전형적인 케이스다.**
...
Istio RequestAuthentication은 `jwksUri`를 지원해요.
...
키가 변경되어도 자동으로 반영되니 완벽한 해결책처럼 보였어요.
...
`connection reset by peer`. 연결이 거부되고 있었어요.
...
왜 connection reset이 발생했는지 파악해봤습니다.

우리 환경에는 `istio-system` 네임스페이스에 STRICT mTLS PeerAuthentication이 걸려있어요.
...
mesh 내부의 mTLS 인증서를 사용하지 않아요.

STRICT mTLS 환경에서는 `jwksUri`를 사용할 수 없다.
이건 Istio의 알려진 제약사항입니다.
...
ExternalSecret이 1시간마다 refresh하고, ArgoCD sync 시 새 키가 자동 반영되는 구조예요.
...
커밋하고 push했습니다.
ArgoCD가 sync를 시작했는데... 문제가 생겼어요.

ArgoCD는 매니페스트를 생성할 때 `helm template`을 사용합니다.
`helm template`은 **클러스터에 접근하지 않아요**.
...
`helm install`이나 `helm upgrade` 때는 동작하지만, `helm template` 때는 항상 빈 값을 반환해요.

**ArgoCD를 쓰는 한 Helm lookup은 사용할 수 없다.**

이건 ArgoCD의 잘 알려진 제약사항입니다.
`argocd-repo-server`에 ServiceAccount와 RBAC을 설정하면 가능하다는 글도 있지만, 보안상 바람직하지 않아요.
...
SSM에 저장한 JWKS는 향후 자동화 파이프라인용으로 보관해요.
...
디버깅 과정에서 `kubectl exec`으로 User 서비스 Pod 내부에서 `wget`을 실행했었죠.
...
Discord 알림 채널에 CRITICAL 메시지가 연달아 쏟아졌어요.

`kubectl exec`을 실행하면 Pod의 컨테이너에서 프로세스가 생성됩니다.
`wget`은 DNS 해석 등을 위해 `/etc/passwd`를 읽을 수 있어요.
Falco가 이걸 **컨테이너 내부에서 민감 파일 접근**으로 탐지한 겁니다.

디버깅하려고 exec한 건데, 보안 도구가 난리를 친 것이다.
...
condition에 `and not (...)` 를 계속 추가하는 방식은 유지보수가 힘들어요.
...
각 예외에 **사유와 날짜**를 주석으로 기록하는 것도 규칙으로 정했어요.
...
하나의 JWT 401을 디버깅하면서 **6개 추가 이슈**가 쏟아진 것이다.

RBAC 403은 Istio AuthorizationPolicy의 `notPaths`가 중간 와일드카드(`/api/*/status`)를 지원하지 않아서 발생한 문제였어요.
pricing 404는 dev와 prod의 UUID가 다른 전형적인 환경 불일치 문제였고요.
...
검증 실패 시 요청을 차단하려면 반드시 `AuthorizationPolicy`로 `require-jwt`를 설정해야 해요.

이 둘이 세트라는 걸 몰랐기 때문에, JWKS가 19개 서비스 전부 틀려있었는데도 아무도 몰랐던 거예요.
**장애가 안 나서 문제를 인지하지 못한 것이 더 위험하다.**

### 2. STRICT mTLS 환경에서 jwksUri는 함정

`jwksUri`는 깔끔한 해결책처럼 보이지만, STRICT mTLS 환경에서는 동작하지 않습니다.
istiod의 JWKS fetcher는 plain HTTP 클라이언트이기 때문이에요.
...
`helm template`은 클러스터에 접근하지 않으므로 `lookup` 함수가 항상 빈 값을 반환해요.

Helm의 동적 기능을 100% 활용하려면 ArgoCD 대신 `helm upgrade`를 직접 사용해야 합니다.
하지만 GitOps의 장점을 포기하는 건 더 큰 손실이에요.

### 4. 보안 강화가 숨은 버그를 드러낸다

AuthorizationPolicy를 추가하자 JWKS 불일치가 드러났습니다.
CDN 경유라는 새로운 경로를 테스트하자 6개 추가 이슈가 드러났고요.

**보안을 강화하고, 새로운 경로를 테스트하는 것이 숨어있던 문제를 드러내는 가장 좋은 방법이다.**

...
그런데 두 타겟이 **같은 이유**로 unhealthy인 것은 아니었습니다.
각각 다른 원인이 있었습니다.
...
커밋 7708a90(#52 리뷰 개선)에서 보안 강화 목적으로 포트 바인딩을 변경한 것이 원인이었습니다:
...
의도는 좋았습니다. 불필요한 외부 노출을 막으려고 localhost로 제한한 것이기 때문입니다.
하지만 **ALB가 EC2의 3000번 포트로 직접 접근하는 구조**를 고려하지 않은 것이 문제였습니다.
...
Grafana와는 별개로, 서버 타겟 그룹도 unhealthy였습니다.
...
Docker 포트 바인딩으로 보안을 강화하려는 시도는 일견 합리적입니다.
...
네트워크 접근 제한은 Docker 포트 바인딩이 아닌 **Security Group**에서 처리하는 것이 올바른 방법입니다.
...
모니터링 스택 배포 후에는 타겟 상태를 반드시 확인해야 합니다:

...
| 항목 | 이전 (문제) | 이후 (해결) |
|---|---|---|
| 흐름 | `Route53 → ALB (Terraform) → Istio Gateway (Helm) → Services` | `Route53 → NLB (Gateway API) → Kubernetes Gateway → HTTPRoute → Services` |
| 프로비저닝 | Helm chart 스키마 오류로 실패 | AWS LB Controller가 자동 생성 |
...
Route53이 이전 NLB를 가리키고 있어 타임아웃이 발생했습니다.
...
최종 흐름은 `Client → HTTPS(443) → NLB(TLS Termination) → HTTP(80) → Gateway → Services`입니다. NLB가 ACM 인증서로 TLS를 종료하고, Gateway는 plain HTTP로 트래픽을 받습니다.
...
![api.wealist.co.kr HTTPS 트래픽 최종 아키텍처](/diagrams/eks-troubleshooting-part3-istio-ambient-2-1.svg)

...
가장 충격적이었던 것은 `seat_statuses` 테이블입니다. 실제 크기는 7.5GB인데 통계상으로는 live tup이 0으로 표기됐습니다. 플래너는 `pg_stat_user_tables`가 아니라 `pg_class`의 reltuples(추정 행 수)로 plan을 세우는데, ANALYZE가 없으면 이 값도 갱신되지 않아 실제 데이터량과 맞지 않는 비효율 plan이 나올 수 있습니다.
...
- **후속 결과**: 2차 부하테스트에서 `ticket_success`는 15.60%에서 13.2%로 오히려 악화됐습니다. MaxConns 축소가 역효과였고, 모니터링 Pod가 Pending 상태에 빠질 만큼 노드 자원이 부족했던 것이 더 큰 원인이었습니다. 자세한 내용은 [3000VU 2차 부하테스트 — 결과 악화 원인과 다음 실행 전 체크리스트](/logs/goti-3000vu-2nd-and-next-checklist)에서 다룹니다.

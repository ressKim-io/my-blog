...
| 항목 | 장점 | 단점/리스크 |
|---|---|---|
| Redis-first | RDS 부하 대폭 감소, latency ms 단위 | Redis 장애 시 영향 범위 큼 → multi-AZ + sentinel/cluster 필수 |
...
이 전환이 실제로 얼마나 빨라졌는지는 이후 D0~D1 롤아웃에서 확인됩니다. [Redis SoT D0~D1 프로덕션 롤아웃 — 558ms를 32ms로](/logs/goti-redis-sot-d0-d1-rollout)에서 seat-statuses 응답이 900ms대에서 32ms로 줄어든 실측치를 다룹니다.
...
- [Cache-Aside 패턴 — Lazy Loading 캐싱과 TTL Self-Healing](/essays/goti-deepdive-cache-aside-pattern)
- [Redis SETNX 분산 락 — 좌석 경합을 메모리 원자 연산으로 끝내는 법](/essays/goti-deepdive-redis-setnx-lock)
- [Redis Pub/Sub — 채널 기반 메시징과 fire-and-forget의 동작 원리](/essays/goti-deepdive-redis-pubsub)
- [Dirty Set과 Write-Behind — 캐시 변경분을 비동기로 영속 저장소에 반영하는 원리](/essays/goti-deepdive-dirty-set-write-behind)

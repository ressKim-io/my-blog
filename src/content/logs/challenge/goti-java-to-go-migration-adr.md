...
이 측정에서 드러난 핵심 수치는 **queue pass_rate 1.04%**입니다. 3000VU 중 99%가 대기열 타임아웃으로 실 예매 단계까지 도달하지 못했습니다. Java 런타임이 목표 트래픽(10만~50만 동시 접속) 대응에 구조적으로 부족하다는 사실이 정량적으로 확인됐습니다. 같은 측정 데이터를 API별로 더 깊이 분석한 결과, 병목의 상당 부분은 Stadium 조회 API의 N+1 쿼리에 있었습니다(자세한 내용은 [KEDA Capacity Planning — 3000 VU 부하테스트에서 drop된 진짜 범인은 Stadium 조회 API](/logs/goti-capacity-planning-keda) 참고). JVM 런타임 자체의 한계와 이 API별 쿼리 문제는 서로 배타적이지 않으며, 이번 ADR의 전면 Go 전환 결정은 두 원인을 모두 해소하는 근거로 작동했습니다.
...
아래는 **Go 런타임 전환 자체의 직접 효과**입니다. PgBouncer, Redis 캐시, ANALYZE 등 다른 최적화와 혼재되지 않는 수치입니다. 표의 pod당 메모리 요청 값은 배포 시 설정한 requests이며, 이 설정치는 앞서 확인한 ticketing-go PoC 실측 메모리 사용량(동일 부하 조건 대비 6배 절감)을 근거로 산정했습니다.
...
- [Go 런타임 — 정적 컴파일·콜드스타트·GC의 동작 원리](/essays/goti-deepdive-go-runtime)
- [Go 동시성 — goroutine·channel·GMP 스케줄러의 동작 원리](/essays/goti-deepdive-go-concurrency)
- [distroless 컨테이너 이미지 — OS 패키지 없는 경량·보안 이미지의 원리](/essays/goti-deepdive-distroless-container-image)

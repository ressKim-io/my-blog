---
title: "로그인 세션 수시 만료 — 근본 원인 분석 (P0~P2 8건 + 5개 완화책)"
excerpt: "로그인 직후 세션이 튕기고 탭을 비활성화했다가 돌아오면 로그아웃되는 증상을 근본 원인까지 추적했습니다. 클라이언트 세션 1시간과 서버 JWT 30분의 미스매치가 가장 유력한 P0였고, 이차 원인 4건과 관찰 사항 2건, 완화책 5개를 우선순위대로 정리했습니다"
...
> "로그인 직후인데 갑자기 세션 만료로 튕긴다", "탭을 잠시 비활성화했다가 돌아오면 로그아웃 상태다" — 두 가지 증상의 근본 원인을 추적했습니다. 가장 유력한 범인은 두 가지(P0)입니다. 첫째는 **클라이언트 세션 수명 1시간과 서버 JWT exp 30분의 미스매치**, 둘째는 **auto-reissue 경로가 3개 path만 대상으로 해서 적용 범위가 좁다**는 점입니다. 그 외 이차 원인 4건(P1)과 관찰 사항 2건(P2)을 정리하고 완화책 5개를 우선순위대로 제시합니다
...
**체감과의 대응**: 사용자가 "로그인 직후"라고 느끼는 시점은 사실 30분이 지난 뒤입니다. 백엔드가 401을 반환하고 reissue가 실패하거나 비대응 경로로 들어가면서 `/session-expired`로 튕기는 흐름입니다
...
- **부작용**: 서버 JWT는 이미 만료됐는데 클라이언트는 "아직 1시간 남음"으로 판단합니다. reissue를 시도하지 않고 그대로 401을 노출하니 앱이 어색한 상태로 멈춥니다
...
- **사용자 증상은 데이터로 매핑해 확정합니다** — "로그인 직후처럼 느낀다"가 실제로는 "30분 경과 후 401"이라는 것을 측정값으로 확인하지 않으면 완화책이 엉뚱하게 작동합니다. 체감만 듣고 "세션 로직이 이상한 것 같다"고 방향을 잡으면 다른 곳을 고치게 됩니다
- **양쪽 설정이 일치하지 않는 곳은 항상 의심합니다** — 클라이언트 1시간과 서버 30분처럼 두 레이어 수명이 다르면 특정 시점에 반드시 미스매치가 드러납니다. 한쪽을 바꿀 때 다른 쪽도 명시적으로 함께 조정해야 합니다
- **`clearAuth` 전 reissue 1회는 저비용 완화책입니다** — 만료 시 곧바로 로그아웃 처리하지 않고 reissue를 한 번 시도하면, "예상 밖 만료" 체감 대부분이 사라집니다
- **localStorage persist 키는 최소화합니다** — `isManualLogout`처럼 상태 분기에 쓰는 플래그가 persist되면 탭 간·세션 간 edge case가 폭발적으로 늘어납니다. 메모리 전용으로 유지할 수 있는지 먼저 검토합니다
- **백그라운드 탭의 timer throttle을 기억합니다** — `setInterval`로 세션 타이머를 돌릴 때, 복귀 시점에 음수로 점프하는 값을 `<= 0` 한 번으로 잘라버리면 사용자 체감은 "나도 모르는 사이 로그아웃"이 됩니다. 가시성(`visibilitychange`) 이벤트와 함께 재계산 로직을 넣습니다

...
excerpt: "3개 대기열 구현체를 1000VU 동일 조건으로 부하 비교했습니다. 폴링 1초는 처리량 최고, 5초+heartbeat는 Redis 효율 최고, 2초는 균형형으로 결과가 나뉘었습니다"
category: challenge
tags:
  - go-ti
  - Queue
...
> 3개 대기열 구현체(A·B·C)를 1000VU 동일 조건으로 비교했습니다. 폴링 1초 구현은 처리량이 2배 높은 대신 Redis 부하가 최대였고, 폴링 5초+heartbeat 분리 구현은 Redis 부하가 1/3 수준으로 가장 효율적이면서 timeout 0건을 기록했습니다. 폴링 2초 구현은 중간 포지션이지만 JWE 토큰 오버헤드로 enter가 느렸습니다

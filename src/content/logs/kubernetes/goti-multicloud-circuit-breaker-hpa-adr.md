...
흐름은 Client → Worker → AWS(TCP timeout 10s) → GCP(성공) 순으로, 사용자는 매 요청마다 10초를 대기해야 했습니다.
...
- [서킷 브레이커 — 실패를 격리하는 3-상태 머신의 동작 원리](/essays/goti-deepdive-circuit-breaker)
- [Kubernetes HPA — 메트릭 기반 replica 자동 조정 원리](/essays/goti-deepdive-kubernetes-hpa)

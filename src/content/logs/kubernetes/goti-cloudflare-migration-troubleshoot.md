...
CloudFront 배포 `EOLI11PF51LX9`의 설정을 확인했습니다. Custom Error Response가 두 개 등록되어 있었습니다.

- 403 → `/index.html` (200으로 변환)
- 404 → `/index.html` (200으로 변환)
...
문제는 이 규칙이 **Distribution 전역**으로 적용된다는 것입니다. 흐름을 단계별로 따라가면 다음과 같습니다.

1. Backend가 `404 {"error": "User not found"}` JSON을 돌려줍니다
2. CloudFront가 404 코드를 감지해 `index.html`로 응답을 바꿉니다 (200)
3. 클라이언트는 JSON을 기대했는데 HTML을 받아 파싱 에러가 발생합니다
...
DNS NS를 Cloudflare로 전환하면서 Proxy(Orange Cloud)가 ON인 상태에서 문제가 발생했습니다. 사이클은 다음과 같이 돕니다.

1. Client가 Cloudflare에 도달 — Cloudflare가 SSL을 종단합니다 (Flexible 모드, Origin에는 HTTP로 전송)
2. CloudFront가 HTTP 요청을 받아 HTTPS로 리다이렉트합니다 (SSL 종단 #2)
3. Cloudflare가 리다이렉트를 따라가 다시 HTTP로 보냅니다 (1번으로 회귀)
4. 무한 301 루프
...
1. Pages에서 커스텀 도메인(`dev.go-ti.shop`)을 제거합니다
2. Workers route를 `dev.go-ti.shop/*` 전체로 변경합니다
3. Worker 코드에서 `/api/*`는 Kind PC로, 나머지는 Pages `*.pages.dev`로 분기합니다
...
`dev-api.go-ti.shop`을 `resshome.iptime.org`로 가는 CNAME으로 추가하고, Cloudflare Proxy ON + SSL Flexible 모드를 적용했습니다.
...
1. **Worker 자체가 동작하는가?** — `return new Response('OK')`로 확인
2. **Origin에 도달하는가?** — 응답 시간으로 확인(751ms = 도달했다는 신호)
3. **Origin이 올바르게 응답하는가?** — curl로 직접 호출해 응답 비교
4. **Workers와 curl의 차이는 무엇인가?** — Host 헤더 처리 차이로 좁혀짐

...
> VPC CNI max-pods 불일치를 고치려고 Rolling Update를 실행했더니, Terraform SG 삭제 → 서브넷 IP 소진 데드락 → Kyverno 차단 → Prometheus OOM까지 4건이 연쇄로 터졌습니다.
...
Pod이 `Init:0/2` 상태에서 멈추고, IP는 `<none>`입니다.
이상한 점은 10개 노드(t3.large) 중 대부분이 여유 있는데도 발생했다는 것입니다.

특히 `ap-northeast-2c` 존 노드에서 집중적으로 발생했습니다.
...
`ENABLE_PREFIX_DELEGATION=true`가 이미 활성화되어 있었습니다.
...
**35개?** Prefix delegation이 활성화되어 있는데 왜 35입니까?

문제는 **Bottlerocket AMI**였습니다.

일반 Amazon Linux 2 AMI는 VPC CNI가 max-pods를 자동 계산하지만, Bottlerocket은 kubelet의 `max-pods` 설정을 **별도로 지정**해야 합니다. Launch template에 user_data가 없어서 Bottlerocket 기본값 35(secondary IP 모드 기준)가 적용되고 있었습니다.

이 불일치가 만든 상황을 정리하면:

![VPC CNI Prefix Delegation과 Kubelet max-pods 불일치](/diagrams/eks-troubleshooting-part9-rolling-update-cascading-1.svg)
...
노드가 하나씩 교체되면서 새 설정이 적용되는 방식입니다.

여기까지는 괜찮았습니다. 문제는 이 Rolling Update 과정에서 터졌습니다.
...
2단계 full apply에서는 별도로 관리하던 `aws_security_group_rule` 리소스를 destroy하면서 — **AWS에서 실제 규칙이 삭제**되어 버렸습니다.
...
별도 `aws_security_group_rule`을 destroy하면 **AWS API에서 해당 규칙 자체를 삭제**합니다.
...
**교훈**: Terraform에서 Security Group 규칙은 반드시 **한 가지 방법**으로만 관리해야 합니다. inline ingress와 `aws_security_group_rule`을 혼용하면 destroy 시 실제 규칙이 삭제될 수 있습니다.
...
문제는 VPC CNI의 `WARM_PREFIX_TARGET=1` 설정입니다.
...
![서브넷 IP 소진 데드락](/diagrams/eks-troubleshooting-part9-rolling-update-cascading-2.svg)
...
ASG가 자동으로 새 노드(max-pods=110)를 생성하면서 교체가 완료되었습니다.
...
**교훈**: `/24` 서브넷(254 IP)에서 prefix delegation을 사용하면, Rolling Update 중 노드 수가 2배로 뛸 때 IP가 터집니다. 서브넷을 `/20` 이상으로 확장하거나, `WARM_IP_TARGET`을 사전에 낮춰야 합니다.
...
Kyverno admission webhook이 EKS managed node group의 노드 교체 과정에서 **pod 생성을 차단**한 것입니다.
spot 노드 그룹은 이미 완료되었는데, core 노드 그룹에서만 발생했습니다.

복구는 기존 core 노드 EC2를 수동 종료하는 방식으로 우회했습니다.
ASG가 새 노드(max-pods=110)를 자동 생성하면서 결과적으로 교체가 이루어졌습니다.
...
Rolling Update 동안 노드가 오르락내리락하면서 **WAL(Write-Ahead Log)이 대량 축적**되었습니다.
...
같은 규칙을 두 곳에서 관리하면 destroy 시 실제 AWS 규칙이 삭제될 수 있습니다.
...
**둘째, 서브넷 설계는 Day 1에 결정해야 합니다.**
`/24`는 개발 환경에선 충분하지만, prefix delegation + Rolling Update가 만나면 순식간에 바닥납니다.
프로덕션 서브넷은 최소 `/20` 이상으로 잡는 것이 안전합니다.

**셋째, Terraform은 "선언적"이지만 "안전"하지는 않습니다.**

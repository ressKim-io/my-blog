...
excerpt: "커널은 메모리를 크게, 그리고 게으르게 내어 줍니다. 그 덩어리를 객체 단위로 잘게 쪼개 파는 소매상이 malloc입니다. glibc ptmalloc의 arena·chunk·bin, 락을 건너뛰는 tcache, brk와 mmap을 가르는 M_MMAP_THRESHOLD, 분명히 free했는데 RSS가 내려오지 않는 현상의 진짜 이유까지 — 2부에 심어 둔 'Rust가 brk/mmap을 직접 호출한다'는 복선이 여기서 완전히 닫힙니다"
category: runtime
tags:
  - rust
  - go
...
> [10편](/essays/kernel-lazy-allocation-page-fault)에서 커널의 계약을 확인했습니다. 커널은 메모리를 크게, 그리고 게으르게 내어 준다는 것
> 문제는 프로그램이 실제로 다루는 객체가 대부분 작다는 것입니다. 문자열 하나, 노드 하나, 버퍼 하나 — 이 작은 것들을 매번 커널에 달라고 할 수는 없습니다
> 그 사이를 메꾸는 계층이 이번 편의 주인공입니다. 그리고 2부부터 끌고 온 복선, "Rust가 `brk`/`mmap`을 직접 호출한다"는 문장이 왜 부정확했는지가 여기서 완전히 닫힙니다

## 커널이 도매라면 malloc은 소매입니다

[10편](/essays/kernel-lazy-allocation-page-fault)에서 잰 커널 비용을 먼저 꺼내 놓겠습니다
...
웹 서버가 요청 하나를 처리하면서 문자열과 노드와 버퍼를 수천 개 만듭니다. 이것을 전부 커널에 직접 요청한다면, 객체 하나마다 시스템 콜 한 번에 페이지 폴트가 붙습니다
...
10편이 도매(커널)였다면 이번 편은 소매(malloc)입니다
...
첫째, **최소 `n`바이트짜리 연속된 메모리 블록**의 시작 주소를 돌려줍니다. 둘째, 그 주소는 일정한 경계에 **align(정렬, CPU나 SIMD 명령어 접근 시 성능 저하와 예외가 발생하지 않도록 메모리 주소를 특정 배수 경계에 맞추는 작업)** 되어 있습니다

이것이 전부입니다. glibc 64비트 환경의 기본 정렬은 **16바이트**입니다(`MALLOC_ALIGNMENT = 2 × sizeof(size_t)`)
...
glibc가 쓰는 malloc의 이름은 **ptmalloc(POSIX Thread Malloc, 멀티스레드 환경에서 락 경합을 줄이기 위해 독립된 메모리 풀인 arena 구조를 도입한 glibc의 기본 동적 메모리 할당자)** 이고 구체적인 구현체 버전은 **ptmalloc2**입니다
...
그림의 바깥 보라색 상자가 **arena(아레나, 멀티스레드 간의 락 경합을 줄이기 위해 스레드 그룹마다 할당되는 독립적인 메모리 관리 구획 및 락 단위)** 입니다. 오른쪽 위에 붙은 `mutex` 배지가 핵심입니다

arena는 "메모리 풀 하나 + 그 풀을 지키는 락 하나"를 묶은 단위입니다

그 안의 노란색 상자가 **heap**, 곧 커널에서 실제로 받아 온 연속된 땅입니다

heap 안의 작은 하늘색 조각들이 **chunk**, 손님에게 파는 최소 단위입니다. 오른쪽 끝 점선 상자(**top chunk(탑 청크, arena 힙 영역의 최상단에 위치하여 아직 개별 chunk로 분할되지 않고 남은 대형 미분할 메모리 구역)**)는 아직 아무에게도 팔지 않은 미분할 구역입니다
...
thread arena는 `mmap`으로 받은 구획을 필요한 만큼만 `mprotect`로 접근 가능하게 켜 가며 씁니다. 10편에서 본 예약(reserve) → 사용(commit) 패턴 그대로입니다
...
이것이 **boundary tag(경계 태그, 앞 chunk가 해제되었을 때만 사용되는 prev_size 필드를 앞 chunk가 사용 중일 때는 페이로드 데이터 영역으로 빌려 쓰는 메모리 최적화 기법)** 기법입니다. 말로는 헷갈리니 이웃한 두 chunk를 바이트 단위로 늘어놓고 보겠습니다
...
1. **tcache(Thread Cache, 스레드 로컬 영역에 크기별로 chunk를 보관하여 락 없이 즉시 메모리를 할당하고 해제하는 초고속 소형 캐시)** — 스레드마다 하나씩 갖는 **락 없는** 개인 캐시입니다. 24바이트부터 1,032바이트까지를 16바이트 간격으로 나눈 **64개의 빈**을 두고, 빈 하나에 최대 **7개**의 chunk를 LIFO로 쌓아 둡니다. 락을 잡지 않으므로 대부분의 소형 할당·해제가 여기서 곧장 끝납니다. **glibc 2.26(2017)** 이후 malloc이 빨라진 결정적 이유입니다
2. **fastbin(패스트 빈, 소형 chunk 해제 시 속도를 위해 이웃 chunk와의 병합을 미루고 LIFO 방식으로 빠르게 보관하는 단일 연결 리스트)** — tcache가 꽉 찼거나 담지 못하는 작은 chunk를 받습니다(기본 상한 128바이트). 속도를 위해 이웃 chunk와의 **병합을 일부러 미루고** 단일 연결 리스트에 LIFO로 쌓습니다. 빠른 대신 조각을 남기는 전략입니다
3. **unsorted bin(언소티드 빈, 해제된 chunk를 즉시 분류하지 않고 임시 대기열에 모아 두었다가 다음 할당 요청 시 한 번에 순회하며 분류하고 재활용하는 지연 정렬 리스트)** — 방금 free된 chunk가 잠시 대기하는 이중 연결 리스트 한 줄입니다. ptmalloc은 정리를 미뤄 두는 대신, 다음 `malloc`이 이 대기열을 훑고 지나가며 "마침 크기가 맞으면 바로 쓰고, 아니면 small·large bin으로 제자리 정리"를 합니다. 일종의 지연 분류입니다
4. **small bin(스몰 빈, 1,024바이트 미만의 소형 chunk를 16바이트 간격의 정확한 크기 단위로 고정하여 관리하는 FIFO 방식 이중 연결 리스트)** — 빈 하나가 **정확히 한 크기**만 담습니다(16바이트 간격, 1KB 미만). 같은 크기 요청이 오면 검색 없이 바로 꺼내 줄 수 있어 빠르고, FIFO로 동작합니다
5. **large bin(라지 빈, 1,024바이트 이상의 대형 chunk를 특정 크기 범위 그룹으로 묶어 best-fit 탐색 및 건너뛰기 포인터로 관리하는 이중 연결 리스트)** — 여기서부터는 빈 하나가 크기의 **범위**를 담습니다. 그래서 그 안에서 요청에 가장 가까운 것을 골라야 하는데(best-fit), 이를 빠르게 하려고 크기순으로 정렬해 두고 `fd_nextsize`·`bk_nextsize`라는 건너뛰기 포인터로 크기 그룹을 넘나듭니다
...
그림 아래 붉은 점선이 그 **coalescing(병합, 인접한 메모리 블록들이 모두 해제되었을 때 이를 단일 대형 연속 블록으로 합쳐 외부 단편화를 해소하는 과정)** 입니다
...
[3편](/essays/mn-scheduler-go-java-rust)의 이야기와 나란히 놓으면 대비가 선명해집니다
...
tcache·fastbin의 연결 리스트, 그리고 jemalloc·mimalloc의 락 없는 free list는 모두 **CAS(Compare-And-Swap, 메모리의 현재 값이 예상 값과 일치할 때만 새로운 값으로 원자적 변경하여 락 없이 동기화를 수행하는 CPU 명령어)** 에 기대어 여러 스레드가 락 없이 같은 자료구조를 만집니다

CAS는 "이 자리의 값이 내가 본 그 값이면 새 값으로 바꿔라"를 명령 하나로 처리하는 원자적 연산입니다. 락 대신 이것을 써서 경합을 넘깁니다
...
x86-64는 **TSO(Total Store Order, 전체 저장 순서, 다중 코어 환경에서도 각 CPU가 쓰기 작업을 수행한 순서가 다른 CPU들에게 사실상 그대로 보장되는 강한 메모리 일관성 모델)**, 곧 강한 모델입니다. 재배열이 거의 없어서 평범한 적재·저장만으로도 대체로 순서가 지켜집니다
...
여기에 **false sharing(거짓 공유, 서로 다른 스레드가 동일한 CPU 캐시라인 내의 서로 다른 변수를 수정할 때 하드웨어 레벨에서 캐시라인 전체가 무효화되어 심각한 병목을 일으키는 현상)** 이 겹치면 성능 차이가 드러납니다
...
매번 `mmap`과 `munmap` 시스템 콜이 나가면 그것이 다시 커널 왕복 비용입니다. 상각하려고 만든 계층이 상각을 포기하는 셈입니다
...
- **내부 단편화(internal)** — 요청 크기를 16바이트나 크기 클래스 경계로 반올림하면서 생기는 자투리입니다. 앞서 본 `malloc(17)`이 32바이트를 차지하던 그 낭비입니다. 개별로는 작아도 객체 수억 개가 쌓이면 무시할 수 없습니다
...
RSS는 실제로 물리 프레임이 붙어 있는 양이라고 [10편](/essays/kernel-lazy-allocation-page-fault)에서 봤습니다. 반환되지 않은 이 영역은 계속 RSS로 잡힙니다

오래 사는 서버에서는 이런 "박힌 못" 하나 때문에 반환이 막히는 일이 흔합니다. 트래픽이 빠져 free를 다 해도 RSS 그래프가 피크에 붙어 내려오지 않는 진짜 이유가 이것입니다

그리고 cgroup은 VSZ가 아니라 RSS를 셉니다. 10편에서 확인한 이 회계 규칙 때문에, 반환되지 않은 자투리가 컨테이너 메모리 한도를 그대로 잡아먹습니다
...
10편의 질문이 "커널은 어떻게 주는가"였다면, 이번 편의 질문은 이것입니다
...
| **Go** | 우회 (자체 할당자) | 런타임이 `mmap`을 직접 호출 | 정적 바이너리, [12편](/essays/go-allocator-mcache-contiguous-stack)에서 해부 |
| **Java (JVM)** | 객체는 우회 (GC 힙 · TLAB) | JVM이 `mmap`을 직접 호출 | JNI·Direct 버퍼는 `malloc`, [13편](/essays/jvm-tlab-bump-pointer-offheap)에서 |
...
그리고 세 상자 모두 아래쪽 커널로 내려갑니다. 결국 물리 프레임을 내어 주는 것은 커널뿐이기 때문입니다

오른쪽 붉은 점선을 눈여겨볼 필요가 있습니다. Java가 GC 힙을 건너뛰고 libc malloc으로 곧장 가는 우회로이고, 뒤에서 다시 다룹니다

### Rust — 복선의 완전한 회수

Rust의 기본 전역 할당자는 **`System`**, 곧 플랫폼 libc의 `malloc`/`free`입니다. 리눅스라면 방금까지 뜯어본 glibc ptmalloc입니다
...
실무적으로 이것이 왜 중요한가 하면, **Rust 서비스의 메모리 성격(단편화, tail latency, RSS 반환)이 상당 부분 그 아래 깔린 malloc 구현체에 좌우된다**는 뜻이기 때문입니다

[5편](/essays/rust-aot-zero-cost-codegen)에서 본 "제로코스트 추상화"는 언어 차원의 오버헤드를 없앤 것이지, 힙의 거동까지 언어가 쥐었다는 뜻이 아닙니다

위 코드에서 `#[global_allocator]` 세 줄을 바꾸는 것만으로 서비스의 RSS 곡선이 달라지는 이유가 이것입니다

### Go와 Java — 도매 계층을 직접 짓다

**Go** — libc를 링크하지 않는 정적 바이너리라 malloc 계층을 통째로 우회합니다. 런타임이 직접 `mmap`으로 큰 arena를 받아 자체 할당자로 나눕니다([12편](/essays/go-allocator-mcache-contiguous-stack))

`cgo`로 C 코드를 부를 때에만 그 부분이 libc malloc을 탑니다

**Java(JVM)** — 자바 객체는 GC 힙 안의 **TLAB**에서 나오므로 malloc을 우회합니다([13편](/essays/jvm-tlab-bump-pointer-offheap))

하지만 JVM 자신은 C++로 짜여 있고, **JNI**와 **DirectByteBuffer**, Netty 같은 off-heap 버퍼는 native malloc을 씁니다. 앞 그림의 붉은 점선이 이 경로입니다

그래서 자바 컨테이너에서 `-Xmx`로 힙을 제한해도 막지 못하는 RSS가 존재합니다. [9편](/essays/java-jit-inversion-conditions)에서 "힙은 60%인데 OOMKilled"라고 불렀던 그 현상의 한 축이 바로 여기입니다

그리고 이 native malloc에는 앞서 본 arena 폭발이 그대로 적용됩니다. JVM 컨테이너 튜닝에서 `MALLOC_ARENA_MAX`가 단골로 등장하는 이유가 두 사실이 겹치기 때문입니다

### 수렴점

네 진영 모두 결국 10편의 커널 계약 위에 서 있습니다. `brk`/`mmap`, demand paging, overcommit — 아무도 여기서 벗어나지 못합니다
...
반대로 Go와 JVM은 이 도매 계층을 스스로 소유해 언어 고유의 의미(가비지 컬렉션, 고루틴 스택)와 완전히 통합했습니다. 대신 [4편](/essays/runtime-weight-zero-cost-vs-fat)에서 잰 런타임의 무게를 짊어졌습니다

**"언어가 가볍다"와 "메모리가 예측 가능하다"는 서로 다른 축의 이야기입니다.** 이것이 이번 편이 남기는 한 문장입니다

## DevOps 관점에서 더 파고들 질문

1. **arena 폭발 확인** — 코어 32개짜리 노드의 컨테이너에서 RSS가 예상보다 높습니다. 원인이 glibc arena 폭발(코어 수 × 8)인지 어떻게 확인할까요(`/proc/PID/smaps`의 heap 세그먼트 개수, `MALLOC_ARENA_MAX`를 바꿔 가며 비교)? 그리고 컨테이너의 CPU 쿼터와 glibc가 인식하는 온라인 코어 수는 왜 어긋날까요?
2. **반환 안 되는 RSS** — 트래픽이 줄어도 RSS가 내려오지 않을 때, 원인이 glibc의 top chunk 반환 제약인지 어떻게 검증할까요? `malloc_trim()`과 jemalloc의 `dirty_decay_ms` 중 무엇으로 대응하는 것이 맞을까요?
3. **할당자 선택의 증명** — glibc·jemalloc·mimalloc 중 하나를 고를 때 p99 tail latency, 단편화율(RSS ÷ 실사용량), 처리량 중 어디에 가중치를 둬야 하며, 그 판단을 어떤 도구(`jeprof`, `malloc_stats`, `malloc_info`)로 증명할까요?
4. **Rust 할당자 교체의 효과** — `#[global_allocator]`를 jemalloc으로 바꾼 뒤 "좋아졌다"를 어떻게 수치로 보일까요? 할당이 병목이 아닌 서비스라면 왜 차이가 거의 없을 수 있을까요?
5. **JVM RSS 분리 계측** — JVM 컨테이너의 OOMKilled를 조사할 때, `-Xmx`로 잡은 자바 힙과 JNI·Direct 버퍼가 쓰는 native malloc RSS를 어떻게 분리해서 계측할까요(NMT: Native Memory Tracking, `pmap`, `jcmd`)? 여기에 `MALLOC_ARENA_MAX`가 왜 끼어들까요?
6. **ARM 서버의 스케일링** — AArch64(Graviton·Ampere)에서 malloc 집약 워크로드의 스케일링이 x86과 다르게 나타난다면, 약한 메모리 모델의 배리어 비용과 캐시라인(64B, Apple은 128B) false sharing 중 무엇을 먼저 의심하고 어떻게 관측해야 할까요?
...
특히 [6편](/essays/go-aot-fast-build-tradeoff)에서 이스케이프 분석이 "이건 힙으로 보낸다"고 판정했던 그 객체가 실제로 `mcache`의 어느 span에 떨어지는지, 그 착지 지점을 끝까지 추적하겠습니다

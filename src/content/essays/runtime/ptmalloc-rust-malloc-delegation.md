...
그림의 바깥 보라색 상자가 **arena**입니다. 오른쪽 위에 붙은 `mutex` 배지가 핵심으로, arena는 "메모리 풀 하나 + 그 풀을 지키는 락 하나"를 묶은 단위입니다
...
chunk 크기는 항상 16의 배수라 하위 4비트가 늘 0입니다. 이 놀고 있는 비트에 메타데이터를 실어 보냅니다
...
tcache·fastbin의 연결 리스트, 그리고 jemalloc·mimalloc의 락 없는 free list는 모두 **원자적 연산(CAS)** 에 기대어 여러 스레드가 락 없이 같은 자료구조를 만집니다
...
문제는 왼쪽입니다. 붉은 점선 아래가 전부 free인데도 붉은 괄호대로 **묶여 있습니다**(stuck). `brk`는 위에서부터만 내릴 수 있어서, 살아 있는 chunk 하나를 지나쳐 내려갈 수 없기 때문입니다
...
| **Go** | 우회 (자체 할당자) | 런타임이 `mmap`을 직접 호출 | 정적 바이너리, 3.3에서 해부 |
| **Java (JVM)** | 객체는 우회 (GC 힙 · TLAB) | JVM이 `mmap`을 직접 호출 | JNI·Direct 버퍼는 `malloc`, 3.4에서 |
...
**Go** — libc를 링크하지 않는 정적 바이너리라 malloc 계층을 통째로 우회합니다. 런타임이 직접 `mmap`으로 큰 arena를 받아 자체 할당자로 나눕니다(3.3)

`cgo`로 C 코드를 부를 때에만 그 부분이 libc malloc을 탑니다

**Java(JVM)** — 자바 객체는 GC 힙 안의 **TLAB**에서 나오므로 malloc을 우회합니다(3.4)

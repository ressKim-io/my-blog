...
VSZ가 아무리 커도 그것만으로 죽지 않지만, RSS가 한도를 넘으면 죽습니다. [9편](/essays/java-jit-inversion-conditions)에서 본 "힙은 60%인데 OOMKilled"의 물리적 토대가 바로 이 회계입니다. 정밀 해부는 5부에서 합니다
...
5. **max_map_count 장애** — 기본 65,530에 걸려 `mmap`이 `ENOMEM`으로 실패하는 장애를 어떻게 진단할까요(`/proc/PID/maps` 줄 수를 `wc -l`)? Elasticsearch가 262,144를 요구하는 물리적 이유(mmapfs로 세그먼트를 다수 매핑)는 무엇일까요?

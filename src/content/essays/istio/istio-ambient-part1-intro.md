...
category: istio
tags: ["istio", "ambient-mesh", "ztunnel", "waypoint", "service-mesh", "kubernetes", concept]
series:
  name: "istio-ambient"
  order: 1
date: "2025-12-22"
...
지금까지 배운 Istio는 모두 **Sidecar 방식**이었습니다. 각 Pod마다 Envoy Sidecar가 함께 배포되어 트래픽을 처리했습니다. 하지만 2024년 11월, Istio 1.24에서 **Ambient Mode**가 정식(GA) 출시되었습니다.
...
Sidecar 방식에서는 모든 Pod에 Envoy 프록시가 주입됩니다. Pod가 100개면 Sidecar도 100개가 필요합니다. 반면 Ambient 방식에서는 Node마다 하나의 ztunnel만 배포됩니다. 5개 Node 클러스터라면 ztunnel은 5개뿐입니다.
...
학습하면서 궁금했던 것들입니다:
- Sidecar 없이 어떻게 mTLS가 가능합니까?
- ztunnel과 waypoint는 무엇입니까?
- 왜 Ambient를 도입해야 합니까?
...
**업그레이드 문제**가 가장 큽니다. Istio 버전을 1.23에서 1.24로 올린다고 가정해보겠습니다. 새 Envoy 버전을 적용하려면 모든 Pod를 재시작해야 합니다. 1000개 Pod 클러스터에서 Rolling Restart를 하면 수 시간이 걸릴 수 있고, 그 동안 서비스 안정성에 영향을 줄 수 있습니다.
...
기존 Sidecar는 L4(TCP)와 L7(HTTP)을 모두 처리했습니다. mTLS 암호화도 하고, HTTP 헤더 기반 라우팅도 하고, JWT 검증도 했습니다. 그런데 생각해보면, 모든 서비스가 L7 기능을 필요로 하지는 않습니다.

내부 gRPC 통신만 하는 서비스라면 mTLS만 있으면 충분합니다. 굳이 무거운 Envoy가 HTTP 파싱까지 할 필요가 없습니다. Ambient는 이 점에 착안해서 역할을 분리했습니다.
...
Sidecar 방식에서는 모든 Pod에 Envoy가 주입되어 L4/L7을 모두 처리했습니다. 리소스 오버헤드가 크고, 업그레이드할 때마다 모든 Pod를 재시작해야 했습니다.

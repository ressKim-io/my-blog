...
category: istio
tags: ["istio", "ambient-mesh", "ztunnel", "waypoint", "l4-l7", "sidecar", "kubernetes", concept]
series:
  name: "istio-ambient"
  order: 2
date: "2025-12-23"
...
1. mTLS가 L4에서 처리된다는 게 무슨 의미입니까?
2. 언제 ztunnel만으로 충분하고, 언제 waypoint가 필요합니까?
3. Sidecar 방식과 Ambient 방식의 트래픽 경로 차이는 무엇입니까?
4. HBONE 프로토콜은 무엇입니까?
...
왜 이렇게 분리하는 것입니까? 실제 서비스 대부분은 mTLS 암호화만 필요하고 HTTP 라우팅 같은 L7 기능은 필요 없습니다. 분리하면 L4만 필요한 서비스는 가벼운 ztunnel만 거치고, L7이 필요한 서비스만 waypoint를 추가로 배포합니다.
...
왜 별도의 터널링 프로토콜이 필요합니까? Sidecar 방식에서는 Envoy가 Pod 내부에 있으므로 iptables로 Pod의 모든 트래픽을 가로챌 수 있습니다. 하지만 ztunnel은 Node 레벨에서 동작합니다. Pod A의 트래픽이 Pod B로 가려면 Node A의 ztunnel에서 Node B의 ztunnel로 명시적인 터널을 통해 전달해야 합니다.

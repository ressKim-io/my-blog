...
전체 실패 사슬을 단계별로 따라가면 다음과 같습니다

1. kubelet이 STS 토큰을 포함해 API 서버에 요청
2. API 서버가 Authenticator webhook으로 위임 → IAM Role 매칭 성공
3. username 템플릿 `system:node:{{EC2PrivateDNSName}}` 렌더링 시작
4. 클러스터 역할로 `ec2:DescribeInstances` 호출 → **403 UnauthorizedOperation** (AmazonEKSClusterPolicy 누락)
5. 템플릿 렌더링 실패 → 인증 실패 → 401 Unauthorized

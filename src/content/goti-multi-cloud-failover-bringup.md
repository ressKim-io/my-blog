...
전체 흐름을 단계별로 풀면 다음과 같습니다.

1. CF Worker가 `ctx.waitUntil(Promise.allSettled([...]))`로 두 sink에 동시에 POST를 날립니다 — `https://aws-api.go-ti.shop/__worker-metrics/ingest`와 `https://gcp-api.go-ti.shop/__worker-metrics/ingest`
2. 각 cluster의 Istio Gateway가 `/__worker-metrics` 경로를 받아 `goti-load-observer`로 라우팅합니다
3. `goti-load-observer`가 메트릭을 노출하고 Prometheus가 scrape합니다 (GCP는 향후 적용 예정)
4. AWS Grafana 대시보드 "Multi-Cloud Failover"(17 panel)가 양쪽 데이터를 모두 시각화합니다

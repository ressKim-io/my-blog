...
**상세 가이드**: `.Codex/plans/diagram-conversion/best-practices.md` (1.1) — 색상 토큰·접근성 메타·실작업 패턴 9종(사이클/데드락/워터폴/UI 모킹 처리/ADR Before-After/카테고리·시리즈 중복/트리 정책/SVG 작성 주의/평탄화 우선순위)
...
상세 스타일 가이드: `.Codex/Draw-io-*.md` 참조
...
> 기술 블로그 글 작성 시 Codex가 따라야 할 규칙 (브라우저/Codex 공통)
...
코드블록 안에 `┌┐└┘├┤│─` 박스나 `→←` 화살표로 다이어그램을 그리는 패턴은 1.x 본문 정리(`/.Codex/plans/ascii-cleanup/`)에서 224편 중 141편에 211블록을 평탄화한 결과 **재발 방지가 필요**하다고 판단했습니다. 신규 글 작성 시 다음 정책을 따릅니다.
...
5·6번 SVG 경로를 택할 때는 `.Codex/plans/diagram-conversion/best-practices.md` (1.1) §1(색상 매핑)과 §11(실작업 패턴 9종)을 참고합니다. 작업 폴더는 51편 / 49 SVG 변환 결과를 누적해 둔 reference입니다.

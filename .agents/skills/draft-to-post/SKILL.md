...
- `category`는 AGENTS.md에 정의된 6개 중 하나만 사용.
...
[MUST] 문장·문단·bullet 끝 마침표(.) 생략 — 한국어 블로그 관행
       예외: URL, 버전(1.24.0), 소수점, 약어(e.g.), 코드블록 내부 문법
       물음표(?)/느낌표(!)는 유지
...
원본 draft에 다음 **신호**가 있으면 `🧭 선택지 비교` 섹션을 추가합니다.

- "Option A/B/C", "안1/안2", "~~를 고려했으나", "~~대신 ~~를 선택"
- 도구/아키텍처/롤아웃 순서/기술 스택 선정이 본문 주제
- 해결 경로가 여러 개였고 하나를 골랐다는 기록

**없으면 쓰지 않습니다.** 원본에 대안 언급이 없는 단일 원인→해결 글(typo, env 누락, 버전 불일치)에는 넣지 않습니다. **원본에 없는 대안을 만들어내지 않습니다** — 이는 사실 훼손입니다.

위치:
- 트러블슈팅: `🔥 문제 → 🤔 원인 → 🧭 선택지 비교 → ✅ 결정 + 이유 → 📚 배운 점`
- ADR: 기존 구조 유지(`배경 → 선택지 → 결정 → 근거`)

상세 규칙·구조·예시·체크리스트는 `./decision-tradeoff.md` 참조. 신호가 있으면 이 파일을 Read하고 작성합니다.
...
**다이어그램 안의 텍스트는 반드시 영어**로 작성합니다. 한글은 폰트 문제로 깨집니다. 설명은 이미지 아래 본문에 한글로 자세히 씁니다. 상세 스타일은 `.Codex/Draw-io-*.md` 참조.

## 시리즈 판단 (중요 — 반드시 series-plan.md 참조)

시리즈 소속·slug·order는 `.Codex/plans/series-plan.md`가 **단일 기준**입니다. 추측·즉석 판단 금지. 변환 전 반드시:

1. `.Codex/plans/series-plan.md`를 Read로 확인
...
- **시리즈 플랜(단일 기준)**: `.Codex/plans/series-plan.md` — 모든 미변환 draft의 시리즈/order 매핑
- 프로젝트 규칙 원본: `AGENTS.md` (Blog Content Stats, 블로그 글 작성 지침서)

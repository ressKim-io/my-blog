...
# 대기열 구현체 최종 선정: member-a (CDN 캐싱) 채택

## Context

대규모 티켓팅 서비스의 대기열 구현체를 3명(팀원 B/팀원 A/팀원 C)이 각각 POC로 개발. 1000 VU + 3000 VU 부하테스트를 통해 성능 비교 후 최종 1개를 선정해야 함.

- 테스트 환경: EKS prod (10노드, Karpenter), ALB 직접 + Cloudflare CDN
- 공통 조건: QUEUE_MAX_CAPACITY=100, setup() 토큰 일괄 발급
- 테스트 일정: 2026-04-03 (ALB 직접 3개 구현체), 2026-04-04 (CDN 캐싱 member-a)

## Issue

### Option A: member-b — Redis Sorted Set + 1초 Polling
...
### Option B: member-a — JWE 토큰 + 분산 Lock + CDN 캐싱
...
### Option C: member-c — Redis ZSET + Heartbeat + 5초 Polling
...
**최종 선택: Option B — member-a (CDN 캐싱)**

선택 근거:

1. **CDN 캐싱이 결정적 차별점**: polling 요청을 Cloudflare 엣지에서 흡수 → origin Redis 부하 대폭 감소. 3000 VU에서 status avg 220ms는 member-c(158ms)과 비슷한 수준이면서, origin 부하는 훨씬 적음
2. **확장성**: VU 증가 시 CDN 캐시 히트율이 높아져 origin 부하는 거의 일정. member-c은 선형 스케일이지만 여전히 origin 직접 부하
...
트레이드오프:
- member-c이 안정성(timeout 0)에서는 우위지만, CDN 캐싱 없이는 대규모 확장 시 Redis가 결국 병목
- member-a의 JWE 오버헤드는 enter 시에만 발생하고, 핵심 polling 경로는 CDN이 흡수

## Result

### 후속 작업

- [ ] member-a 구현체를 develop 브랜치에 머지 (POC → 정식)
...
- [ ] 다른 POC 브랜치(member-b, member-c) 정리
...
- `Goti-load-test/scenarios/queue-member-a.js` — K6 부하테스트 스크립트
- `Goti-load-test/results/cdn-member-a-3000vu-kimhj.json` — CDN 3K 테스트 결과

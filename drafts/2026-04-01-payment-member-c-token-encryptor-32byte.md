...
project: Goti-server (payment-member-c)
tags: [aes-256, token-encryptor, secret-key, ssm, queue-poc]
---

# payment-member-c CrashLoop — QUEUE_TOKEN_SECRET_KEY 31바이트 (32바이트 필요)

## Context
EKS rolling update 후 서비스 복구 과정에서 goti-payment-member-c-prod만 CrashLoopBackOff 지속.
팀원 C님 POC 브랜치(`poc/queue-member-c-loadtest`)에만 존재하는 `TokenEncryptor` 클래스가 원인.
...
3. `poc/queue-waiting-member-c`, `poc/queue-member-c-loadtest` 두 브랜치 모두 동일 코드 확인
...
```bash
kubectl delete secret goti-payment-member-c-prod-secrets -n goti
kubectl rollout restart deploy goti-payment-member-c-prod -n goti
```

## Result
- payment-member-c-prod 정상 기동 확인 (HikariPool 연결 성공, Redisson 시작)
- goti namespace 15/15 전체 ready=true
- 기존 발급 토큰은 키 변경으로 무효화됨 (POC 환경이라 영향 없음)

## Related Files
- `Goti-server/common/src/main/java/com/goti/global/utils/TokenEncryptor.java` (poc/queue-member-c-loadtest 브랜치)

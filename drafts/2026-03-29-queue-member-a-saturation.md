# Queue POC (팀원 A) Saturation 부하 테스트 결과

테스트일: 2026-03-29
테스터: ress
도구: K6
대상: goti-queue (팀원 A 구현체) — 대기열 진입 → 폴링 → 좌석 선택 → 결제 E2E

---

## 1. 테스트 목적

팀원 A Queue POC의 처리 한계(saturation point)를 파악한다.
...
| goti-queue (팀원 A) | 1 pod (replica=1) | CPU 100m-500m, Memory 512Mi-1Gi |
...
| 3 | 팀원 B/팀원 C POC 동일 조건 비교 테스트 | A/B/C 구현체 성능 비교 데이터 | P1 | ress |
| 4 | maxCapacity 튜닝 (100 → 150/200) | 대기열 회전율 향상, 대기 시간 감소 | P2 | 팀원 A |
| 5 | 분산락 최적화 (TTL/재시도 간격 조정) | enter 지연 감소 | P2 | 팀원 A |

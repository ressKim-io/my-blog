# 팀원 A POC Saturation 부하테스트 결과

테스트일: 2026-03-30
도구: K6
대상: goti-queue-dev (poc-member-a-24-2c7348a)
경기: KIA vs 두산 (cffa774c-5231-4b5d-b994-0c291898cdef, 4/1)

## 버그 수정 사항 (테스트 과정에서 발견 → 수정)

### 1. Lua Script ARGV 직렬화 불일치 (poc-member-a-22)
...
### 2. updateSeatEnterMeta race condition (poc-member-a-23)
...
### 3. Redis 직렬화 혼용 JsonParseException (poc-member-a-24)
...
- **queue_status p95 2.18s**: pod 과부하로 status polling 느림 (팀원 B 390ms 대비 높음)

## 팀원 A vs 팀원 B 1000 VU 비교

| 지표 | 팀원 A | 팀원 B | 차이 | 비고 |
|------|------|------|------|------|
| **iterations** | **5,495** | 3,713 | **+48%** | 팀원 A이 iteration 회전 빠름 |
| **iterations/s** | **14.5** | 9.84 | **+47%** | 팀원 A 처리량 우위 |
| **ticket_success** | 81.51% | **97.96%** | -16pp | 팀원 B이 예매 성공률 높음 |
| **queue_pass_rate** | 81.56% | **100%** | -18pp | 팀원 B은 전원 통과 |
| queue_wait avg | **56.4s** | 67.7s | **-17%** | 팀원 A이 대기 시간 짧음 |
| queue_wait p95 | **1m17s** | 136.4s | **-15%** | 팀원 A이 꼬리 지연도 적음 |
| queue_poll avg | **27회** | 52.2회 | **-48%** | 팀원 A이 polling 횟수 절반 |
| queue_status p95 | 2.18s | **390ms** | 5.6x 높음 | 팀원 A pod 과부하 |
| http_req p95 | 2.18s | **390ms** | 5.6x 높음 | 동일 원인 |
| http_req_failed | 0.51% | **0.013%** | 39x 높음 | 팀원 A 504 간헐 발생 |
| immediate_pass | 1.81% | 2.21% | 유사 | 첫 100명 즉시 통과 |

### 핵심 차이 분석

**팀원 A 강점 — 처리량(throughput)**
- iterations 48% 더 많음: 팀원 A의 "동적 publishedRank 계산 + 즉시 승격" 방식이 슬롯 회전을 빠르게 함
- polling 횟수 48% 적음: leave 즉시 availableSlots 증가 → 다음 status에서 바로 통과
- 대기 시간 17% 짧음

**팀원 B 강점 — 안정성(reliability)**
- ticket_success 97.96% vs 81.51%: 팀원 B은 예매 실패가 거의 없음
- queue_pass_rate 100%: 대기열 통과 후 전원 예매 성공
- http_req_failed 0.013%: API 에러 거의 0
- status p95 390ms: 안정적 응답 시간

**차이 원인 추정**
1. **팀원 A ticket_success 낮은 이유**: 빠른 회전 → 동시 좌석 경합 증가 → AVAILABLE 좌석 소진/hold 실패
2. **팀원 A status 느린 이유**: publishedRank를 매 요청마다 동적 계산 (Redis HGET 2회 + 계산) vs 팀원 B은 Scheduler가 주기적 갱신
3. **팀원 A http_failed 높은 이유**: 1000 VU × 1초 polling = 1000 req/s status 요청이 pod 1개 한계 초과

### 공정 비교 시 고려사항

- **pod 수**: 양쪽 모두 1 replica (공정)
- **maxCapacity**: 양쪽 모두 100 (공정)
- **polling 간격**: 팀원 A 1초, 팀원 B 1초 (통일 완료)
- **경기/좌석 데이터**: 다른 경기 사용 — 좌석 수 차이 가능
- **이미지 태그**: 팀원 A poc-member-a-24, 팀원 B poc-member-b-17
...
## 팀원 A vs 팀원 B 전체 비교 (300/1000/3000 VU)

### 1000 VU

| 지표 | 팀원 A | 팀원 B | 승자 |
|------|------|------|------|
| iterations | **5,495** | 3,713 | **팀원 A +48%** |
| ticket_success | 81.51% | **97.96%** | **팀원 B** |
| queue_pass_rate | 81.56% | **100%** | **팀원 B** |
| queue_wait avg | **56s** | 67.7s | **팀원 A -17%** |
| polling 횟수 | **27회** | 52회 | **팀원 A -48%** |
| status p95 | 2.18s | **390ms** | **팀원 B** |
| http_failed | 0.51% | **0.013%** | **팀원 B** |

### 3000 VU

| 지표 | 팀원 A | 팀원 B | 승자 |
|------|------|------|------|
| iterations | **6,574** | 763 | **팀원 A 8.6x** |
| iterations/s | **12.2** | 2.0 | **팀원 A 6.1x** |
| ticket_success | 56.85% | **86.89%** | **팀원 B** |
| queue_pass_rate | 57.90% | **100%** | **팀원 B** |
| queue_wait avg | 1m55s | **53.5s** | **팀원 B** |
| queue_status p95 | **2.13s** | 3,103ms | **팀원 A** |
| http_req_failed | 0.63% | **0.003%** | **팀원 B** |
| HTTP reqs | **445K** | 329K | **팀원 A +35%** |
| active/100 유지 | **98~100** | - | **팀원 A** |

### 종합 분석

**팀원 A (방식 3) 특성:**
- **처리량 최강**: 3000 VU에서 팀원 B 대비 8.6x iterations. 동적 publishedRank 계산으로 leave 즉시 다음 유저 승격
- **수용인원 100% 유지**: active=98~100/100으로 슬롯 낭비 없음
- **대규모 VU에서 status API 안정**: 3000 VU에서도 status p95 2.13s (팀원 B 3.1s 대비 안정)
...
- Scheduler 주기(1초)보다 **팀원 B의 API 레이턴시 급증**이 핵심 → status p95 3.1s에서 polling 자체가 느려짐
- 팀원 A은 status API가 3000 VU에서도 p95 2.13s로 안정 유지

**결론**: 팀원 A=고처리량/고효율, 팀원 B=고안정성/고신뢰성. 대규모(3000+)에서 팀원 A이 압도적 처리량 우위.

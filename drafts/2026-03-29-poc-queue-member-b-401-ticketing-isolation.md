...
# POC Queue 팀원 B 401/403 — SecurityConfig 미등록 + 공유 ticketing pod 의존성 격리

## Context

3개 대기열 구현체(팀원 A/팀원 B/팀원 C) A/B 비교 부하테스트 진행 중.
- 환경: Kind K8s (Istio service mesh, ArgoCD GitOps)
- 팀원 A POC: 300/1000/3000 VU saturation 테스트 완료
- 팀원 B POC: `POST /api/v1/queue/member-b/validate` 호출 시 401 empty body로 실패

팀원 B 구현체는 `poc/queue-waiting-member-b` 브랜치에서 `queue` 모듈을 독립 Spring Boot 앱으로 배포.
Istio VirtualService가 `/api/v1/queue/member-b/**` → rewrite `/api/v1/queue/**` → goti-queue-member-b-dev pod.
...
```
POST /api/v1/queue/member-b/validate → 401 (empty body)
```
...
**가설**: Istio JWT 검증 실패 → 결과: Istio requestAuthentication 설정은 팀원 A과 동일, 팀원 A은 정상 → 기각
...
- goti-ticketing-dev 이미지(`dev-44-3737148`)에 **팀원 A POC의 ReservationSession 검증 코드**가 포함
  - `#329` (poc/queue-waiting-member-a-cdn-optimized)가 develop에 머지된 상태
  - seat-grades: ReservationSession 없으면 403
  - seat-sections: 세션 만료 시 400
- 팀원 A 테스트 시에는 팀원 A queue가 세션을 발급하므로 정상 동작
- 팀원 B queue는 다른 세션 방식(QueueInterceptor + X-Queue-Token) → 팀원 A 세션 검증 통과 불가
...
- `goti-ticketing-member-b-dev` 신규 배포 (팀원 B 브랜치의 api 모놀리스 이미지)
- Istio 라우팅: `/member-b/api/v1/**` → rewrite `/api/v1/**` → goti-ticketing-member-b-dev
- K6: `ticketingBase = baseUrl + "/member-b"` 으로 ticketing 호출 분리
- Redis DB 1 공유 (queue와 동일 DB → QueueInterceptor 토큰 검증 가능)

## Result

### 1차/2차 수정 후
- `POST /api/v1/queue/member-b/validate` → **200 OK** (isPassed=true, token 발급)
- pod 정상 기동 (2/2 Running, restart 0)
- smoke 1 VU: `queue_pass_rate: 100%`, `queue_validate_ms p95: 314ms`

### 3차 수정
- Goti-k8s: `goti-ticketing-member-b/values.yaml` 신규 + ApplicationSet element 추가
...
### goti-server (poc/fix-queue-member-b-security-config)
...
### Goti-k8s (main)
- `environments/dev/goti-ticketing-member-b/values.yaml` — 신규
- `gitops/dev/applicationsets/goti-msa-appset.yaml` — ticketing-member-b element 추가

### Goti-load-test (feature/queue-member-b)
- `scenarios/queue-member-b.js` — ticketingBase 도입, sections 동적 조회
- `scenarios/queue-member-b-saturation.js` — 동일

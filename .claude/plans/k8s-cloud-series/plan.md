...
> 캐시 가설의 직접 검증(작업 집합 크기를 파라미터로 한 스윕)은 미수행 — 본문에서
> 기전을 단언하지 말고 "워크로드 의존"까지만 쓸 것
...
**미측정 (다음 단계)**:
- 작업 집합 크기 스윕으로 §8.5 정정본의 캐시 가설 직접 검증
  (`lock-probe`에 `--footprint` 추가 → 오버스크립션 유지율이 작업 집합에 따라 어떻게 변하는지)

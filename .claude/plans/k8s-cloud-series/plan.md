...
**확정 사실 5 — steal은 세금을 과소 계상한다**: 처리량 손실 65.9%인데 `steal`은 50.3%다.
15.6pp 격차의 원인은 nproc 12가 물리 12코어가 아니라 6코어 × SMT2라는 데 있다.
solo 게스트의 8 vCPU는 코어를 거의 독점하지만 oversub에서는 24 vCPU가 물리 6코어에 얹힌다.
`steal`은 vCPU 스케줄 대기만 세고 SMT 형제 간섭은 세지 못한다 (§8.7에서 핀 고정으로 분리 예정)
...
**미측정 (다음 단계)**: §8.8 vCPU 핀 고정으로 §8.5 확정 사실 5의 SMT 격차 분리

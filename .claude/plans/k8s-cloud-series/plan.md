...
|---|---|---|---|
| `none` | 55.1% | 54.8% | **−0.3 pp** |
| `kspin` | 83.0% | 79.1% | −3.9 pp |
| `mutex` | 119.3% | 99.8% | −19.5 pp |
| `spin` | 120.7% | 149.8% | +29.1 pp |

**확정 사실 6 — 현대 커널 기본값에서 가상화 고유의 몫은 작다**: 락 도메인까지 일치시킨
대조군으로 재면 독립 CPU 부하(`none`)의 가상화 추가 비용은 **−0.3 pp**로 사실상 0이다.
커널 스핀락 경합(`kspin`)도 −3.9 pp에 그친다. 게스트당 처리량이 절반이 되는 주된 이유는
"가상화 세금"이 아니라 **CPU를 2배로 요구했으니 절반을 받는 산수**다.
`mutex`(−19.5 pp)가 가장 큰데, futex 깨우기가 vCPU 스케줄을 기다려야 하기 때문이다.
`spin`이 L1에서 오히려 빨라지는 것(+29.1 pp)은 호스트 선점이 실효 병렬도를 낮춰
...
|---|---|---|---|---|
| `none` | 54.8% | 54.9% | 1.0× | 대조군 — 변화 없음 |
| `spin` | 149.8% | 126.8% | 1.2× | 유저스페이스 락, 커널 완화 대상 아님 |
| `mutex` | 99.8% | 70.9% | 1.4× | futex 내부 커널 락 경유 |
| `kspin` | **79.1%** | **1.69%** | **46.8×** | 커널 스핀락 — LHP 직격 |

`kspin`의 게스트별 처리량은 22,671 / 54,782 / 149,935 it/s로 **편차 6.6배**다.
공평 분배가 붕괴한다. `none`과 `spin`이 대조군으로서 변화가 없다는 점이
이 실험의 내적 타당성을 보증한다

**확정 사실 8 — 파국의 순간에도 게스트 지표는 침묵한다**: `nopvspin` + `kspin` 조합에서
처리량이 47배 무너지는 동안 게스트의 `run_delay_ratio`는 0.000478, `cpu_efficiency`는
0.9996이다. 게스트 커널은 자기가 건강하다고 보고한다. `steal` 50.2%만이 이상을 알리는데,
그 값조차 "절반을 뺏겼다"고 말할 뿐 **처리량이 47배 무너졌다는 사실은 전달하지 못한다**.

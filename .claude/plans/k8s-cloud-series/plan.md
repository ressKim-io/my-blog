...
> 락 경합이 아니라 **프로브 자신의 거짓 공유를 측정**했다. 12스레드 30초 동일 조건에서
> `spin` 2,431,040 → 7,888,358(3.24×), `kspin` 4,583,332 → 6,018,636(1.31×),
> `none`은 1.00×(무영향 — 반복마다 레지스터 연산 200회라 카운터가 묻힌다).
> 수정: `__attribute__((aligned(64)))` + `aligned_alloc(64, ...)`.
> **아래 수치는 전부 수정 후 재측정본**이며, 오염된 원자료는 감사용으로
> `measure/results/contaminated-false-sharing/`에 보존했다.
...
| 모드 | L0 유지율 (`1x8`→`3x8`) | L1 유지율 (`solo`→`oversub`) | 차이 |
|---|---|---|---|
| `none` | 54.9% | 55.0% | **+0.1 pp** |
| `kspin` | 83.5% | 71.5% | **−12.1 pp** |
| `mutex` | 120.1% | 99.2% | −20.9 pp |
| `spin` | 72.7% | 91.9% | +19.2 pp |

**확정 사실 6 — 가상화 고유의 몫은 워크로드가 무엇을 경합하느냐에 달렸다**:
락 도메인까지 일치시킨 대조군으로 재면 독립 CPU 부하(`none`)의 가상화 추가 비용은
**+0.1 pp로 사실상 0**이다. 게스트당 처리량이 절반이 되는 주된 이유는 "가상화 세금"이 아니라
**CPU를 2배로 요구했으니 절반을 받는 산수**다.

반면 락이 끼면 달라진다. 커널 스핀락 경합(`kspin`)은 **−12.1 pp**, futex 파킹(`mutex`)은
**−20.9 pp**를 추가로 문다. `mutex`가 가장 큰 것은 깨우기가 vCPU 스케줄을 기다려야 하기
때문이다. `spin`이 L1에서 오히려 빨라지는 것(+19.2 pp)은 호스트 선점이 실효 병렬도를 낮춰
핫 락의 캐시 라인 왕복을 줄이기 때문이다.

> **거짓 공유 교정 전후**: `none`(−0.3 → +0.1)과 `mutex`(−19.5 → −20.9)는 결론이 유지되나
> `kspin`은 **−3.9 → −12.1 pp로 3배 커졌다**. 교정이 5편의 LHP 논지를 오히려 강화한다
...
|---|---|---|---|---|
| `spread` | 물리 6코어에 하나씩 (`0,1,2,3,4,5`) | 28.56 M it/s | 3.79 M it/s | 4,444 MHz |
| `packed` | 물리 3코어에 형제까지 (`0,6,1,7,2,8`) | 29.96 M it/s | 4.32 M it/s | 5,039 MHz |

**확정 사실 11 — 부스트 클럭이 SMT 측정을 뒤집는다**: 벽시계 기준으로는 SMT를 **공유하는**
`packed`가 오히려 4.9%(`none`)~14.1%(`kspin`) 빠르다. 원인은 SMT가 아니라 주파수다 —
...
|---|---|---|---|---|---|
| `none` | 19.79 M it/s | 19.81 M it/s | **+0.1%** | 1.020× | 1.005× |
| `kspin` | 2.87 M it/s | 3.45 M it/s | **+20.1%** | 1.016× | 1.004× |

**확정 사실 12 — 핀 고정은 락 경합 워크로드에만 듣는다**: 독립 CPU 부하(`none`)에서는
개선이 +0.1%로 사실상 0이다. 커널 스핀락 경합(`kspin`)에서만 +20.1% 개선된다.
...
**부분적으로만 맞다**. 독립 CPU 부하에는 이득이 없고(+0.1%), 락 경합 워크로드에는 +20.1%로
의미가 있다. "파국을 원천 방지"가 아니라 "락 경합이 있는 워크로드에서 20% 회복하고 공평성을

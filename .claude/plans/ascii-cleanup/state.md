...
- [x] `CLAUDE.md` 재발 방지 룰 추가 — "ASCII 다이어그램 정책 (재발 방지)" 섹션 신설, 박스+5줄 금지 표 + 평탄화 우선순위 5단계 + 자가 점검 절차 (2026-04-30)
- [x] `scan.mjs` 재실행 — Before/After 비교 (2026-04-30, `inventory.before.md` 보존)
- [x] `decisions.md` 작성 (G1~G41 + 묶음 처리 로그 누적 — Phase 3 진행 중 매 그룹 작성 완료)
- [x] 작업 폴더 보존 (git 추적, audit 패턴) — Phase 1부터 모든 산출물 git 추적 중

### Before/After 정량 비교 (scan.mjs 재실행)

| 항목 | Phase 1 시작 (2026-04-28) | Phase 4 종료 (2026-04-30) | 차이 |
|---|---|---|---|
...
- **flatten 추천 211 → 38**: Phase 3에서 171건을 평탄화하고 40건을 keep 보정한 결과. 남은 38건은 keep 보정으로 처리한 블록 중 자동 분류기가 여전히 `arrow-only`/`misc`로 잡는 경우(lang=`text`/`hcl`/`promql` 명시되어 있어도 사람 판단이 필요)가 대부분. `criteria.md`의 분류 보정 룰을 `scan.mjs`에 자동화하면 더 줄일 수 있음 (다음 세션 후보)
- **글 -35편**: 평탄화로 ASCII 블록이 모두 사라진 글 35편이 인벤토리에서 제외됨
- **keep 105 유지**: 디자인 개편 세션 입력 자산 그대로 보존
- **skip +9**: 평탄화 과정에서 lang을 추가 명시한 코드 블록(`bash`/`yaml`/`json` 등)이 늘어 `code-arrow`로 재분류된 결과
...
- 2026-04-30: Phase 4 마무리. CLAUDE.md에 "ASCII 다이어그램 정책 (재발 방지)" 섹션 신설(박스+5줄 금지 표 + 평탄화 우선순위 5단계 + 자가 점검). `scan.mjs` 재실행 결과 141편/429블록/4,425줄 → 106편/265블록/3,707줄로 감소(글 -25%, 블록 -38%, flatten 추천 -82%). 백업본 `inventory.before.md/json` 보존하여 Before/After git diff 가능. **Phase 1~4 전 단계 완료. 다음 세션은 디자인 개편(다크모드 제거 + keep 105 블록 시각화 시스템 결정)**.

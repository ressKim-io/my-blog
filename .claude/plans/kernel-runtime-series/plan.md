...
### 4.1 문체·구조
- 격식체 100%, 문장 끝 마침표 생략 등 **CLAUDE.md 절대 규칙 전부 적용**
- draft 맨 앞의 "**이 문서의 성격**" 블록쿼트는 **삭제**하고, rust-cs-layer 스타일의
  시리즈 소개 블록쿼트로 교체:
  ```markdown
...
4. 정보량이 본문과 중복되는 장식성 ASCII → 삭제
- 편당 SVG는 대략 3~6개 예상. 다이어그램 안 텍스트는 한글 가능(SVG는 폰트 문제 없음)
...
|---|-------|-------------|-------|------|
| 1.1 | `1부_1.1_유저모드_커널모드_syscall비용.md` | `syscall-mode-switch-cost` | 1 | ☐ |
...
3. 본문 변환 (`src/content/essays/runtime/{slug}.md`)
4. SVG 작성 (`public/diagrams/{slug}-{n}.svg`)
5. blog-reviewer 에이전트로 규칙 검증 + `npm run build` 통과 확인
6. **이 문서 §6 상태 갱신** + 커밋 (`docs(content): runtime 시리즈 N편 — {제목}`)
